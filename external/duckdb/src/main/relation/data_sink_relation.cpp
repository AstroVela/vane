...
DataSinkRelation::DataSinkRelation(shared_ptr<Relation> child_p, string operation_id_p)
	...
	if (operation_id.empty()) {
		throw InvalidInputException("DataSink operation identity must not be empty");
	}
	if (operation_id.size() > 256) {
		throw InvalidInputException("DataSink operation identity must be at most 256 UTF-8 bytes");
	}
