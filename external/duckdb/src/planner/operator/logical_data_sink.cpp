...
static BoundStatement BindDataSinkOperatorExtension(ClientContext &context, Binder &binder, OperatorExtensionInfo *info,
...
PhysicalOperator &LogicalDataSink::CreatePlan(ClientContext &context, PhysicalPlanGenerator &planner) {
	(void)context;
	if (children.size() != 1) {
