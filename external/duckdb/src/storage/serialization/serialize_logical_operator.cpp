...
unique_ptr<LogicalOperator> LogicalOperator::Deserialize(Deserializer &deserializer) {
	...
	case LogicalOperatorType::LOGICAL_WINDOW:
		...
		break;
	default:
		throw SerializationException("Unsupported type for deserialization of LogicalOperator: %s (enum=%d)!",
		                             EnumUtil::ToString(type).c_str(), static_cast<int>(type));
	}
...
void LogicalColumnDataGet::Serialize(Serializer &serializer) const {
	...
	serializer.WritePropertyWithDefault<bool>(203, "is_expression_derived", is_expression_derived, false);
}
...
unique_ptr<LogicalOperator> LogicalColumnDataGet::Deserialize(Deserializer &deserializer) {
	...
	deserializer.ReadPropertyWithExplicitDefault<bool>(203, "is_expression_derived", result->is_expression_derived, false);
	return std::move(result);
}
