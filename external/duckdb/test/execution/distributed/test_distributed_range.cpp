...
static distributed::ScanSplitBatch MakeSplitBatch(vector<distributed::ScanSplit> splits) {
...
static distributed::ScanSplitBatch MakeSplitBatch(const distributed::ScanSplit &split) {
...
static vector<Value> ExecuteRangeAssignment(Connection &connection, const distributed::DuckPhysicalPlanRef &worker_plan,
	...
	return ExecuteRangeAssignment(connection, worker_plan, MakeSplitBatch(split), scan_node_id);
}
...
static vector<int64_t> ExecuteIntegerAssignments(Connection &connection, const PlannedRangeScan &planned) {
	...
	for (idx_t split_index = 0; split_index < planned.splits.size(); split_index++) {
		auto values =
		    ExecuteRangeAssignment(connection, planned.worker_plan, planned.splits[split_index], 100 + split_index);
...
TEST_CASE("Distributed integer range plans exact index splits", "[distributed][range]") {
	DuckDB db(nullptr);
	...
	auto non_contiguous = MakeSplitBatch(planned.splits[0]);
	non_contiguous.Merge(MakeSplitBatch(planned.splits[2]));
...
TEST_CASE("Distributed range installs an explicit empty split", "[distributed][range]") {
	DuckDB db(nullptr);
	Connection connection(db);
	...
	REQUIRE(ExecuteRangeAssignment(connection, planned.worker_plan, planned.splits[0], 300).empty());
}
...
TEST_CASE("Distributed timestamp range splits only indexable intervals", "[distributed][range]") {
	...
	REQUIRE(fixed.splits.size() == 3);
	vector<string> fixed_values;
	for (idx_t split_index = 0; split_index < fixed.splits.size(); split_index++) {
		for (const auto &value :
		     ExecuteRangeAssignment(connection, fixed.worker_plan, fixed.splits[split_index], 400 + split_index)) {
	...
	REQUIRE(calendar.splits.size() == 1);
	REQUIRE_FALSE(calendar.splits[0].empty);
	REQUIRE_FALSE(calendar.splits[0].estimated_cardinality.IsValid());
	auto distributed_values = ExecuteRangeAssignment(connection, calendar.worker_plan, calendar.splits[0], 500);
...
TEST_CASE("Distributed range rejects malformed split payloads", "[distributed][range]") {
	DuckDB db(nullptr);
	Connection connection(db);
	auto planned = PlanRangeScan(db, connection, "SELECT * FROM range(10)", 2);
	auto malformed = planned.splits[0];
	malformed.extension_payload.push_back('\0');
	...
	execution_plan->SetRoot(scan);
	unordered_map<idx_t, distributed::ScanSplitBatch> assignments;
	assignments.emplace(600, MakeSplitBatch(malformed));
	string apply_error;
