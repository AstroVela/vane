...
static void InitializeConsumers(py::class_<DuckDBPyRelation> &m) {
	...
	m.def(
	    ...
	    "Execute the relation into a first-class DataSink and return its WriteSummary", py::arg("sink"), py::kw_only(),
...
void DuckDBPyRelation::Initialize(py::handle &m) {
	...
	relation_module.def("filter", &DuckDBPyRelation::Filter, "Filter the relation object by the filter in filter_expr",
	                    py::arg("filter_expr"));
	relation_module.def("_mark_data_sink", &DuckDBPyRelation::MarkDataSink, py::arg("operation_id"));
	relation_module.def(
	    "map_batches",
