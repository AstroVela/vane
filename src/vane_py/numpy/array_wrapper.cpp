...
struct FileConvert {
	static py::object ConvertValue(Vector &input, idx_t chunk_offset, NumpyAppendData &append_data) {
		auto &client_properties = append_data.client_properties;
		auto &type = input.GetType();
		auto value = input.GetValue(chunk_offset);
		return PythonObject::FromValue(value, type, client_properties);
	}
};
	...
	if (!idata.validity.AllValid()) {
		for (idx_t i = 0; i < count; i++) {
			...
			if (!idata.validity.RowIsValidUnsafe(src_idx)) {
				...
				out_ptr[offset] = vane_py_convert::RegularConvert::template ConvertValue<DUCKDB_T, NUMPY_T>(
...
static bool ConvertColumnRegular(NumpyAppendData &append_data) {
	return ConvertColumn<T, T, vane_py_convert::RegularConvert>(append_data);
}
	...
	if (!idata.validity.AllValid()) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			if (!idata.validity.RowIsValidUnsafe(src_idx)) {
				...
				out_ptr[offset] =
				    vane_py_convert::IntegralConvert::ConvertValue<DUCKDB_T, double>(src_ptr[src_idx], append_data) /
				    division;
				target_mask[offset] = false;
			}
		}
...
void ArrayWrapper::Append(idx_t current_offset, Vector &input, idx_t source_size, idx_t source_offset, idx_t count) {
	...
	case LogicalTypeId::HUGEINT:
		may_have_null = ConvertColumn<hugeint_t, double, vane_py_convert::IntegralConvert>(append_data);
		break;
	case LogicalTypeId::UHUGEINT:
		may_have_null = ConvertColumn<uhugeint_t, double, vane_py_convert::IntegralConvert>(append_data);
		break;
	...
	case LogicalTypeId::TIMESTAMP_NS:
		may_have_null = ConvertColumn<timestamp_t, int64_t, vane_py_convert::TimestampConvertNano>(append_data);
		break;
	case LogicalTypeId::DATE:
		may_have_null = ConvertColumn<date_t, int64_t, vane_py_convert::DateConvert>(append_data);
		break;
	case LogicalTypeId::TIME:
		may_have_null = ConvertColumn<dtime_t, PyObject *, vane_py_convert::TimeConvert>(append_data);
		break;
	case LogicalTypeId::TIME_NS:
		may_have_null = ConvertColumn<dtime_ns_t, PyObject *, vane_py_convert::TimeNSConvert>(append_data);
		break;
	case LogicalTypeId::INTERVAL:
		may_have_null = ConvertColumn<interval_t, int64_t, vane_py_convert::IntervalConvert>(append_data);
		break;
	case LogicalTypeId::VARCHAR:
		may_have_null = ConvertColumn<string_t, PyObject *, vane_py_convert::StringConvert>(append_data);
		break;
	case LogicalTypeId::BLOB:
		may_have_null = ConvertColumn<string_t, PyObject *, vane_py_convert::BlobConvert>(append_data);
		break;
	case LogicalTypeId::BIT:
		may_have_null = ConvertColumn<string_t, PyObject *, vane_py_convert::BitConvert>(append_data);
		break;
	...
	case LogicalTypeId::VARIANT:
		may_have_null = ConvertNested<py::object, vane_py_convert::VariantConvert>(append_data);
		break;
	case LogicalTypeId::UUID:
		may_have_null = ConvertColumn<hugeint_t, PyObject *, vane_py_convert::UUIDConvert>(append_data);
		break;
