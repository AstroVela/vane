...
void RegisterExceptions(const py::module &m) {
	// Keep Vane's exception registry and translator local to this extension so a
	// separately loaded DuckDB binding cannot share or reorder either one.
	// The base class is mapped to Error in python to somewhat match the DBAPI 2.0 specifications
	py::register_local_exception<Warning>(m, "Warning");
	auto error = py::register_local_exception<PyError>(m, "Error").ptr();
	auto db_error = py::register_local_exception<DatabaseError>(m, "DatabaseError", error).ptr();

	// order of declaration matters, and this needs to be checked last
	// Unknown
	py::register_local_exception<PyFatalException>(m, "FatalException", db_error);
	py::register_local_exception<PyInterruptException>(m, "InterruptException", db_error);
	py::register_local_exception<PyPermissionException>(m, "PermissionException", db_error);
	py::register_local_exception<PySequenceException>(m, "SequenceException", db_error);
	py::register_local_exception<PyDependencyException>(m, "DependencyException", db_error);

	// DataError
	auto data_error = py::register_local_exception<DataError>(m, "DataError", db_error).ptr();
	py::register_local_exception<PyOutOfRangeException>(m, "OutOfRangeException", data_error);
	py::register_local_exception<PyConversionException>(m, "ConversionException", data_error);
	// no unknown type error, or decimal type
	py::register_local_exception<PyTypeMismatchException>(m, "TypeMismatchException", data_error);

	// OperationalError
	auto operational_error = py::register_local_exception<OperationalError>(m, "OperationalError", db_error).ptr();
	py::register_local_exception<PyTransactionException>(m, "TransactionException", operational_error);
	py::register_local_exception<PyOutOfMemoryException>(m, "OutOfMemoryException", operational_error);
	py::register_local_exception<PyConnectionException>(m, "ConnectionException", operational_error);
	// no object size error
	// no null pointer errors
	auto io_exception = py::register_local_exception<PyIOException>(m, "IOException", operational_error).ptr();
	py::register_local_exception<PySerializationException>(m, "SerializationException", operational_error);
	...
	{
		auto http_exc = py::register_local_exception<PyHTTPException>(m, "HTTPException", io_exception);
	...
	// IntegrityError
	auto integrity_error = py::register_local_exception<IntegrityError>(m, "IntegrityError", db_error).ptr();
	py::register_local_exception<PyConstraintException>(m, "ConstraintException", integrity_error);

	// InternalError
	auto internal_error = py::register_local_exception<InternalError>(m, "InternalError", db_error).ptr();
	py::register_local_exception<PyInternalException>(m, "InternalException", internal_error);

	//// ProgrammingError
	auto programming_error = py::register_local_exception<ProgrammingError>(m, "ProgrammingError", db_error).ptr();
	py::register_local_exception<PyParserException>(m, "ParserException", programming_error);
	py::register_local_exception<PySyntaxException>(m, "SyntaxException", programming_error);
	py::register_local_exception<PyBinderException>(m, "BinderException", programming_error);
	py::register_local_exception<PyInvalidInputException>(m, "InvalidInputException", programming_error);
	py::register_local_exception<PyInvalidTypeException>(m, "InvalidTypeException", programming_error);
	// no type for expression exceptions?
	py::register_local_exception<PyCatalogException>(m, "CatalogException", programming_error);

	// NotSupportedError
	auto not_supported_error = py::register_local_exception<NotSupportedError>(m, "NotSupportedError", db_error).ptr();
	py::register_local_exception<PyNotImplementedException>(m, "NotImplementedException", not_supported_error);

	py::register_local_exception_translator([](std::exception_ptr p) { // NOLINT(performance-unnecessary-value-param)
		try {
			if (p) {
