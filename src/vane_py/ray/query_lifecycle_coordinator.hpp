...
public:
	...
	void CompleteAbort(const Abort &abort, const Optional<ErrorDiagnostics> &error);

	Optional<Teardown> BeginTeardown(const std::string &query_id);
	void MarkDropping(const Teardown &teardown);
	void CompleteTeardown(const Teardown &teardown, const Optional<ErrorDiagnostics> &error);
...
private:
	...
	struct Attempt {
		...
		size_t waiters = 0;
		bool complete = false;
		Optional<ErrorDiagnostics> error;
	};
