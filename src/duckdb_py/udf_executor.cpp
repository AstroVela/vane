...
static unique_ptr<OwnedArrowTableStreamFactory> ExportOwnedArrowTableStream(const py::object &table) {
	...
	if (ClientContextMutexHeldByCurrentThread()) {
		...
		throw InternalException("direct Arrow table conversion attempted Python export while holding ClientContext lock");
	}
	...
	if (factory->stream.get_schema(&factory->stream, &factory->schema)) {
		auto error = factory->stream.get_last_error ? factory->stream.get_last_error(&factory->stream) : "unknown error";
...
static CollectorOutputLeaseCallbacks MakeCollectorOutputLeaseCallbacks(const py::object &collector,
...
static bool PayloadUsesTaskAdmission(const Value &payload) {
	...
	return backend.first &&
	       (backend.second == "ray_task" || backend.second == "ray_actor" || backend.second == "subprocess_task" ||
	        backend.second == "subprocess_actor");
}
...
static void ValidateDistributedRefBundlePayload(const Value &payload) {
	...
	const bool subprocess_backend =
	    backend.second == "subprocess_task" || backend.second == "subprocess_actor";
	if (!ray_backend && !subprocess_backend) {
		throw InvalidInputException("distributed UDF ref-bundle output contract does not support execution_backend='%s'",
		                            backend.second);
	}
	if (ray_backend) {
	...
	if (mode.second != "local_shm_ref_bundle") {
		throw InvalidInputException(
		    ...
		    "udf_name=%s got=%s",
		    "local_shm_ref_bundle", backend.second,
		    GetStructStringOrDefault(payload, "udf_name", "<unknown>"), mode.second);
	}
}
...
static string DistributedRefBundleContractError(const Value &payload, idx_t submit_id, const py::handle &value) {
	return StringUtil::Format(
	    ...
	    GetStructStringOrDefault(payload, "execution_backend", "<unknown>"),
	    static_cast<unsigned long long>(submit_id), PythonTypeName(value));
}
...
public:
	...
	uint64_t Register(Value payload, vector<LogicalType> output_types, vector<LogicalType> ref_output_types,
		...
		*out_slot = raw;
		UDFDebugLog(StringUtil::Format("register slot=%llu total_slots=%llu",
	...
	void Unregister(uint64_t id) {
		...
		const auto unregister_timeout = UDFUnregisterTimeout();
		{
			...
			UDFDebugLog(StringUtil::Format("unregister_request slot=%llu inflight=%lld slots=%llu",
			                               static_cast<unsigned long long>(id),
		...
		if (g_on_udf_dispatcher_thread) {
			...
			UDFDebugLog(StringUtil::Format("unregister_deferred_on_dispatcher slot=%llu",
			                               static_cast<unsigned long long>(id)));
			return;
		}
		...
		// dispatcher is still accessing slot data (race → SIGSEGV).
		{
			...
			if (!slot->cleanup_complete) {
				...
				if (!slot->cleanup_cv.wait_for(lk, unregister_timeout,
				                               [&slot] { return slot->cleanup_complete; })) {
					auto msg = StringUtil::Format("udf unregister cleanup exceeded hard deadline of %llu ms for slot=%llu",
		...
		// alive through the condition-variable wait and lock destruction.
		{
			...
			ScopedGILReleaseIfHeld release_gil;
			if (!async_shutdown_complete &&
			    !async_shutdown_cv.wait_for(shutdown_lk, unregister_timeout,
...
private:
		...
		while (!stop.load()) {
			...
			if (UDFDebugEnabled() &&
			    (!pending_commands.empty() || has_pending_cleanup_cancel || has_output_lease_releases ||
			     pending_async_shutdown.load() ||
			     debug_loop_tick % 200 == 0)) {
				UDFDebugLog(StringUtil::Format(
				    ...
				    has_async_inflight ? "true" : "false", drain_async ? "true" : "false",
				    needs_gil ? "true" : "false", work_pending.load() ? "true" : "false",
				    python_wakeup_fired ? "true" : "false"));
			}
				...
				if (pending_async_shutdown.load()) {
					...
					if (should_shutdown && async_collector) {
						try {
							...
							RecordDispatcherError(StringUtil::Format("udf async collector shutdown failed: %s", ex.what()));
							async_collector.reset();
						} catch (const std::exception &ex) {
							RecordDispatcherError(StringUtil::Format("udf async collector shutdown failed: %s", ex.what()));
							async_collector.reset();
			...
			if (!did_work) {
				...
				if (UDFDebugEnabled() && (!has_async_inflight || debug_loop_tick % 200 == 0)) {
					UDFDebugLog(StringUtil::Format(
					    ...
					    static_cast<unsigned long long>(debug_inflight_slots),
					    static_cast<unsigned long long>(debug_inflight_total),
					    work_pending.load() ? "true" : "false", wakeup_fired.load() ? "true" : "false"));
				}
	...
	bool UpdateSlotUDFStats_WithGIL(ExecutorSlot &slot) {
		...
		try {
			...
			bool has_output_budget_estimated =
			    ExtractPythonStatsIdx_WithGIL(stats, "udf_output_budget_estimated_bytes",
	...
	void ReleaseTerminalSubmit(ExecutorSlot &slot, idx_t submit_id) {
		...
		UDFDebugLog(StringUtil::Format("release_terminal slot=%llu submit=%llu inflight_before=%lld inflight_after=%lld",
		                               static_cast<unsigned long long>(slot.id),
		                               static_cast<unsigned long long>(submit_id),
		                               static_cast<long long>(inflight_before),
		                               static_cast<long long>(slot.inflight_count.load())));
	}
	...
	bool DeliverOutputEvent(ExecutorSlot &slot, UDFOutputEvent &&event) {
		...
		if (UDFDebugEnabled() &&
			...
			UDFDebugLog(StringUtil::Format(
			    ...
			    has_registered_output_consumer ? "true" : "false",
			    static_cast<long long>(slot.inflight_count.load())));
		}
	...
	void TrackSubmittedPythonRef_WithGIL(ExecutorSlot &slot, idx_t submit_id, py::object &ref,
		...
		async_collector->obj.attr("track_generator_ref")(py::int_(slot.id), py::int_(submit_id), ref,
		                                                        error_context);
		auto inflight_before = slot.inflight_count.load();
		slot.inflight_count++;
		slot.rows_by_submit_id.emplace(submit_id, std::move(rows));
	...
	bool DrainAsyncResults_WithGIL(vector<ExecutorSlot *> &active) {
		...
		if (async_collector) {
			try {
				...
				bool debug_probe_slots =
				    UDFDebugEnabled() && (debug_probe_tick >= 650 || debug_probe_tick % 200 == 0);
				for (auto *slot : active) {
					...
					if (debug_probe_slots) {
						UDFDebugLog(StringUtil::Format(
						    ...
						    static_cast<unsigned long long>(debug_probe_tick),
						    static_cast<unsigned long long>(slot->id),
						    static_cast<unsigned long long>(capacity.rows),
					...
					try {
						...
						if (event_kind == "data") {
							if (tuple_len != 6 || output_request_id.empty() || output_lease_id.empty()) {
								throw InvalidInputException("udf async collector data event is missing output lease identity");
							}
							auto callbacks =
							    MakeCollectorOutputLeaseCallbacks(async_collector->obj, output_request_id,
							                                      output_lease_id);
						...
						if (GetOutputConsumer(*slot, output_consumer)) {
							...
							if (event_kind == "data") {
								...
								continue;
									...
									g_udf_direct_output_arrow_table_conversion_count.fetch_add(1, std::memory_order_relaxed);
									outputs_chunk = ConvertArrowTableToDataChunk(payload, RequireActiveSlotContext(*slot),
									                                             ExpectedDirectOutputTypes(*slot, submit_id));
							}
							unique_ptr<DataChunk> rows_chunk;
							if (slot->is_table_udf) {
...
public:
	...
	InsertionOrderPreservingMap<string> Stats() override {
		...
		result["udf_queued_task_count"] =
		    std::to_string(slot_->udf_queued_task_count.load(std::memory_order_relaxed));
		result["udf_max_running_tasks"] =
		    std::to_string(slot_->udf_max_running_tasks.load(std::memory_order_relaxed));
...
static unique_ptr<UDFExecutor> CreatePythonUDFExecutor(ClientContext &context, const Value &payload, UDFConfig &,
	...
	return make_uniq<DispatchedUDFPythonExecutor>(std::move(execution_payload), std::move(output_types), std::move(ref_output_types),
	                                              std::move(actor_handles));
}
