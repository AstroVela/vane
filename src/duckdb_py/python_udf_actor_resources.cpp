...
static py::dict BuildUDFNode(idx_t node_id, UDFFunctionData &bind_data, ClientContext &context) {
	auto payload_obj = PythonObject::FromValue(bind_data.payload, bind_data.payload.type(), context.GetClientProperties());
	py::dict meta;
		...
		for (idx_t node_id = 0; node_id < bind_nodes.size(); node_id++) {
			...
			if (backend == "subprocess_actor") {
				...
				throw InvalidInputException(
				    "ray_actor UDF execution requires driver-precreated actor handles from a registered query allocation; "
