	...
	for (auto &name_obj : names) {
		...
		for (auto &existing_name : result) {
			if (StringUtil::CIEquals(existing_name, name)) {
				throw InvalidInputException("input_names must be unique (case-insensitive); duplicate name: '%s'", name);
			}
		}
...
static void RegisterVaneScalarFunctionAtomically(
	...
	try {
		context.RunFunctionInTransaction([&]() {
			auto catalog_entry = Catalog::GetEntry<ScalarFunctionCatalogEntry>(
			    context, SYSTEM_CATALOG, DEFAULT_SCHEMA, name, OnEntryNotFound::RETURN_NULL);
			if (!owns_existing_alias) {
				...
				if (!catalog_entry) {
					throw InvalidInputException("Cannot replace owned Vane alias '%s': its catalog entry is missing", name);
				}
				if (catalog_entry->functions.functions.size() != 1 || info.functions.functions.size() != 1) {
					throw InvalidInputException("Cannot replace owned Vane alias '%s': expected exactly one overload", name);
				}
				...
				if (!dynamic_cast<RegisteredUDFFunctionInfo *>(existing_function.function_info.get())) {
					throw InvalidInputException("Cannot replace function '%s': the existing entry is not a registered Vane "
					                            "SQL alias owned by this connection",
					                            name);
				}
				if (!info.functions.functions.front().Equal(existing_function)) {
					throw InvalidInputException(
					    "Cannot replace owned Vane alias '%s' with a different SQL signature", name);
				}
			...
			if (InjectVaneCatalogRegistrationFailureForTesting(name)) {
				throw InternalException("injected Vane catalog registration failure after dependency swap for '%s'", name);
			}
		});
	} catch (...) {
...
static void InitializeConnectionMethods(py::class_<DuckDBPyConnection, shared_ptr<DuckDBPyConnection>> &m) {
	...
	m.def("_create_vane_function", &DuckDBPyConnection::CreateVaneFunctionInternal,
	      "Internal helper used by vane.attach_function for scalar expression UDF SQL registration",
	      py::arg("name"), py::arg("function"), py::arg("parameters") = py::none(),
	      py::arg("return_type") = nullptr, py::kw_only(), py::arg("replace"));
	m.def("_create_vane_batch_function", &DuckDBPyConnection::CreateVaneBatchFunctionInternal,
	      "Internal helper used by vane.attach_function for row-preserving batch UDF SQL registration",
	      py::arg("name"), py::arg("function"), py::arg("input_names"), py::arg("schema"),
	      py::arg("parameters") = py::none(), py::kw_only(), py::arg("batch_size") = py::none(),
	      py::arg("gpus") = py::none(), py::arg("actor_number") = py::none(), py::arg("stateful") = false,
	      py::arg("row_preserving"), py::arg("replace"));
...
shared_ptr<DuckDBPyConnection> DuckDBPyConnection::UnregisterUDF(const string &name) {
	...
	context.RunFunctionInTransaction([&]() {
		...
		if (catalog_type == PythonUDFCatalogType::SCALAR) {
			auto scalar_entry = Catalog::GetEntry<ScalarFunctionCatalogEntry>(
			    context, SYSTEM_CATALOG, DEFAULT_SCHEMA, name, OnEntryNotFound::RETURN_NULL);
			if (!scalar_entry) {
...
shared_ptr<DuckDBPyConnection> DuckDBPyConnection::CreateVaneBatchFunctionInternal(
	...
	py::object normalized_parameters = normalize_parameters(parameters);
	auto parameter_types = ParseVaneSQLParameters(normalized_parameters,
	                                             "parameters is required for batch SQL vane.func registration");
	...
	ScalarFunction scalar_function(name, std::move(parameter_types), LogicalType::ANY, RegisteredVaneUDFExecute,
	                               RegisteredVaneUDFBind, nullptr, nullptr, nullptr, LogicalType::INVALID,
	                               FunctionStability::VOLATILE);
	scalar_function.SetExtraFunctionInfo(make_shared_ptr<RegisteredUDFFunctionInfo>(
	    payload, parsed_input_names, /*allow_named_arguments=*/true));
	scalar_function.SetBindExpressionCallback(LowerRegisteredExpressionUDF);
...
shared_ptr<DuckDBPyConnection> DuckDBPyConnection::RegisterTableUDF(const string &name, const py::function &udf,
	...
	auto default_parallelism = static_cast<idx_t>(TaskScheduler::GetScheduler(context).NumberOfThreads());
	auto payload = BuildPythonUDFPayload(name, udf, schema, return_type_p, ResolveDefaultUDFExecutionBackend(udf),
