...
RayWorkerManager::DrainFteResultHandles(const string &query_id, double timeout_s,
                                        const RayWorkerRuntime::QueryStatus *finished_status,
                                        const std::unordered_set<duckdb::distributed::TaskContext,
                                                                 duckdb::distributed::TaskContextHash>
                                            *task_context_filter,
	...
	std::sort(drained_outputs.begin(), drained_outputs.end(),
	          [](const DrainedOutput &lhs, const DrainedOutput &rhs) {
...
DuckDBResult<std::vector<duckdb::distributed::MaterializedOutput>>
RayWorkerManager::wait_fte_query(
	...
	try {
		...
		auto drain_res = DrainFteResultHandles(query_id, remaining_timeout_s,
		                                       has_finished_status ? &finished_status : nullptr,
