...
duckdb::distributed::python::ray::MaterializePyPayloadToCollection(const py::object &obj,
	...
	if (!ray_mod.is_none()) {
		try {
			...
			auto safe_get =
			    py::module_::import("duckdb.runners.ray.safe_get").attr("resolve_object_refs_blocking");
			for (int depth = 0; depth < 5 && py::isinstance(resolved, object_ref_cls); ++depth) {
				resolved = safe_get(resolved);
			}
		} catch (const py::error_already_set &e) {
			throw duckdb::InternalException(string("Failed to resolve ray.ObjectRef while materializing Python payload: ") +
			                                e.what());
		}
	}
