...
"""Run Vane TPC-H benchmarks on Ray with per-query timeout and local fallback.

Uses Vane's distributed execution engine (Relation API + Ray runner).
Each query runs in a separate subprocess to isolate hangs. Queries that timeout on Ray
are automatically retried with local Vane execution.

Usage:
    # Ray runner with default 120s timeout per query
    python benchmarking/tpch/run_tpch_vane_ray.py \
    ...
    # Local only (same as run_tpch_vane.py)
    python benchmarking/tpch/run_tpch_vane_ray.py \
...
def run_query_local(parquet_folder, qnum, threads):
    """Run a query directly in the current process (local Vane, no Ray)."""
    con = vane.connect()
    if threads:
        con.execute(f"SET threads={threads}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run Vane TPC-H benchmarks (local or Ray with timeout)")
    ...
    parser.add_argument(
        "--threads", default=None, type=int, help="Number of DuckDB engine threads (default: all cores)"
    )
    parser.add_argument(
