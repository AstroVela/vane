...
def _read_row_count(path: str) -> int:
    ...
    import vane as _native

    if os.path.isdir(path):
        glob = os.path.join(path, "*.parquet")
        return _native.sql(f"SELECT count(*) FROM read_parquet('{glob}')").fetchone()[0]
    return _native.sql(f"SELECT count(*) FROM read_parquet('{path}')").fetchone()[0]
