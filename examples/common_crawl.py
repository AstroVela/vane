...
def relation_from_rows(conn: Any, rows: list[dict[str, Any]]) -> Any:
    ...
    return conn.from_arrow(pa.table({key: pa.array([row[key] for row in rows]) for key in keys}))
...
def sample_relation(conn: Any, limit: int) -> Any:
    ...
    return conn.from_arrow(
        pa.table(
            {
                "WARC-Record-ID": pa.array(
                    [row["WARC-Record-ID"] for row in rows],
                    type=pa.string(),
                ),
                "WARC-Type": pa.array(
                    [row["WARC-Type"] for row in rows],
                    type=pa.string(),
                ),
                "WARC-Target-URI": pa.array(
                    [row["WARC-Target-URI"] for row in rows],
                    type=pa.string(),
                ),
                "WARC-Date": pa.array(
                    [row["WARC-Date"] for row in rows],
                    type=pa.string(),
                ),
                "Content-Length": pa.array(
                    [row["Content-Length"] for row in rows],
                    type=pa.int64(),
                ),
                "warc_content": pa.array(
                    [row["warc_content"] for row in rows],
                    type=pa.binary(),
                ),
                "warc_headers": pa.array(
                    [row["warc_headers"] for row in rows],
                    type=pa.string(),
                ),
            }
        )
...
def run(args: argparse.Namespace) -> None:
    ...
    pages_table = pages.to_arrow_table()
    ...
    chunks_table = chunks.to_arrow_table()
    ...
    print(f"\nSource records scanned: {rel.to_arrow_table().num_rows}")
    ...
    preview_rel = conn.from_arrow(embedded_table if embedded_table is not None else chunks_table)
    select_embedding = ", list_count(embedding) as embedding_dim" if embedded_table is not None else ""
    preview_rel.query(
        "chunks",
        f"""
        select
