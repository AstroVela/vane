...
def sample_relation(conn: Any, limit: int) -> Any:
    ...
    table = pa.table(
        {
            "id": pa.array(list(range(len(rows))), type=pa.int64()),
            "path": pa.array([row["path"] for row in rows], type=pa.string()),
            "audio_format": pa.array(["wav"] * len(rows), type=pa.string()),
            ...
            "fallback_transcript": pa.array(
                [row["transcript"] for row in rows],
                type=pa.string(),
            ),
        }
    )
...
def relation_from_dicts(conn: Any, rows: list[dict[str, Any]]) -> Any:
    if not rows:
...
def run(args: argparse.Namespace) -> None:
    ...
    embedded.query(
        ...
        select
            ...
            list_count(embedding) as embedding_dim
        from segments
