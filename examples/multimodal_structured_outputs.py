    ...
    table = pa.table(
        {
            ...
            "source": pa.array([row["source"] for row in rows], pa.string()),
            "question": pa.array([row["question"] for row in rows], pa.string()),
            "answer": pa.array([row["answer"] for row in rows], pa.string()),
...
def count_rows(rel: Any) -> int:
    return int(rel.query("r", "select count(*) from r").fetchone()[0])
...
def append_prompt_output(conn: Any, rel: Any, prompted: Any, output_column: str) -> Any:
    ...
    output = prompted.to_arrow_table()
    ...
    return conn.from_arrow(base.append_column(output_column, output[output_column]))
