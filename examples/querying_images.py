    ...
    table = pa.table(
        {
            ...
            "size": pa.array([row["size"] for row in rows], type=pa.int64()),
            "image_bytes": pa.array(
                [row["image_bytes"] for row in rows],
                type=pa.binary(),
            ),
        }
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    analyzed_table = analyzed.to_arrow_table()
    top_table = save_outputs(
        analyzed_table,
        Path(args.output_dir),
        min(args.top_k, analyzed_table.num_rows),
    )
    top_rel = conn.from_arrow(top_table)
