...
def test_collector_waits_on_callbacks_without_timed_ray_polling():
    source = (Path(__file__).resolve().parents[2] / "duckdb/execution/udf_stream_result_collector.py").read_text()

    assert "self._ray.wait(" not in source
    assert "timeout=0.1" not in source
    assert "self._thread.join(timeout=" in source
    ...
    try:
        ...
        events = _drain_until(
            ...
            predicate=lambda values: any(
                item[2] in {"complete", "error"} for item in values
            ),
        )
...
def test_inflight_block_keeps_item_capacity_from_read_admission():
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if (
                zero_capacity_events
                or driver.acquire_query_output_block_lease.calls
            ):
                break
            time.sleep(0.005)
