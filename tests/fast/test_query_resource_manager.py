...
def test_ray_tasks_receive_unique_resource_lease_slots():
    ...
    assert first.lease.execution_slot_id == (
        f"ray_task:{stage.stage_id}:{first.lease.lease_id}"
    )
...
def test_driver_subset_yields_when_global_winner_is_fte_owned():
    ...
    selected, yielded = manager.try_acquire_next_queued_task(
        {(driver_request.task_id, driver_request.attempt_id)}
    )
...
def test_fte_request_yields_when_global_winner_is_driver_owned():
    ...
    selected, granted = manager.try_acquire_next_queued_task(
        {(driver_request.task_id, driver_request.attempt_id)}
    )
...
def test_descriptor_admission_yields_to_higher_rank_persistent_waiter():
    ...
    descriptor = manager.try_acquire_task_descriptor(
        _task(upstream.stage_id, "fte")
    )
    ...
    snapshot = manager.snapshot()
    assert [
        item["task_id"] for item in snapshot["admission"]["waiting_tasks"]
    ] == [downstream_waiter.task_id]
...
def test_handed_off_child_output_recycles_credit_without_unaccounting_physical_bytes():
    ...
    assert with_both_outputs["output_leases"][second_output.lease.lease_id]["continuation_credit_id"] == credit[
        "credit_id"
    ]
...
def test_continuation_borrow_selects_feasible_idle_credit_with_smallest_live_output_delta():
    ...
    manager.update_stage_state(parent.stage_id, runnable=True, actor_ready=True)
    parents = [
        manager.try_acquire_task(_task(parent.stage_id, partition))
    ...
    historical_credit_id = manager.snapshot()["output_leases"][first_output.lease.lease_id][
        "continuation_credit_id"
    ]
...
def test_cross_stage_idle_credit_output_excess_is_attributed_exactly_once():
    ...
    reservation_stage_id = next(iter(snapshot["continuation_credits"].values()))[
...
def test_continuation_stage_usage_matches_query_usage_for_active_and_idle_borrower():
    ...
    assert sum(
        stage["usage"]["object_store_bytes"]
        for stage in active["stages"].values()
    ) == 35
    ...
    assert sum(
        stage["usage"]["object_store_bytes"]
        for stage in idle["stages"].values()
    ) == 25
...
def test_ray_task_placement_preserves_combined_fte_and_pinned_actor_reservations():
    ...
    pinned = manager.try_acquire_task(
        _task(producer.stage_id, "pinned", node_id="node-a")
    )
...
def test_latent_fte_and_actor_reservations_cap_ray_task_fanout():
    ...
    producer_grants = [
        manager.try_acquire_task(_task(producer.stage_id, partition))
    ...
    reservations = manager.snapshot()["admission"]["downstream_reservations"][
        producer.stage_id
    ]
    ...
    assert sum(
        reservation["resources"]["object_store_bytes"]
    ...
    assert manager.snapshot()["usage"] == _r(
        cpu=9,
        gpu=1,
        heap=38,
        store=10,
...
def test_upstream_fanout_preserves_concurrent_ray_task_and_fte_slots():
    ...
    upstream_grants = [
        manager.try_acquire_task(_task(upstream.stage_id, partition))
        for partition in range(8)
    ]
...
def test_parent_fte_fanout_preserves_shared_fte_slot_across_nested_ray_task():
    ...
    reservations = snapshot["admission"]["downstream_reservations"][
        parent.stage_id
    ]
    ...
    assert cpu_grant.lease.lease_id in {
        credit["borrowed_by_task_lease_id"]
        for credit in final_snapshot["continuation_credits"].values()
    }
