...
def test_stage_collection_does_not_treat_generic_inout_as_python_udf(tmp_path):
    ...
    try:
        ...
        con.execute(
            f"COPY (SELECT i::BIGINT AS x FROM range(2) tbl(i)) "
            f"TO '{path}' (FORMAT PARQUET)"
        )
        con.execute("SET scalar_subquery_error_on_multiple_rows=false")
        relation = con.sql(
            f"SELECT * FROM unnest((SELECT [x, x + 1] FROM read_parquet('{path}')))"
        )
...
def test_stage_collection_preserves_distinct_stage_identity_for_nested_udfs(tmp_path):
    ...
    try:
        ...
        for stage_id, node in metadata_by_stage.items():
            ...
            assert replay_by_stage[stage_id]["payload"]["execution_backend"] == node["udf_payload"][
                "execution_backend"
            ]
    finally:
        con.close()
...
def test_stage_collection_exposes_only_the_new_graph_api():
    # The binding owns this invariant; this smoke assertion makes sure the API
    # does not expose the removed lazy operator-registration method instead.
    con = duckdb.connect()
    try:
        plan = _physical_plan(con.sql("SELECT 1"), con, "graph-api")
        assert hasattr(plan, "collect_execution_stages")
        assert not hasattr(plan, "annotate_ray_udf_credit_operators")
    finally:
        con.close()
