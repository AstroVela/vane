        ...
        "pipelines": [
            {
                ...
                "operator_details": (
                    operator_details
                    if operator_details is not None
                    else [{} for _ in operators]
                ),
                "stage_ids": [],
            }
...
def test_progress_snapshot_includes_query_resource_manager():
    ...
    try:
        ...
        grant = manager.try_acquire_task(
            TaskRequest("q-progress", stage.stage_id, "task-1", "attempt-1", "node-a")
        )
        ...
        assert snapshot["query"]["query_resource_manager"]["stages"][stage.stage_id][
            "active_task_count"
        ] == 1
    finally:
        clear_query_resource_managers()
                ...
                "fragment_executions": {
                    ...
                    "fragment-udf": {
                        ...
                        "partitions": {
                            ...
                            **{
                                str(index): {"state": "SEALED", "waiting_for_execution": True}
...
def test_progress_distinguishes_remote_exchange_sink_from_local_repartition():
    assert shared_progress_mod._display_pipeline_name(
        {
