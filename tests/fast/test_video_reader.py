...
def test_video_frame_source_has_no_removed_count_admission_env_path(monkeypatch):
    import duckdb
    from duckdb.datasource import read_datasource

    monkeypatch.setenv("VANE_VIDEO_SOURCE_UDF_BACKEND", "ray_task")
    monkeypatch.setenv("VANE_VIDEO_SOURCE_UDF_QUEUE_DEPTH", "999")
    monkeypatch.setenv("VANE_VIDEO_SOURCE_UDF_MAX_OUTSTANDING_BATCHES", "8")
    con = duckdb.connect()
...
def test_video_frame_source_ignores_removed_ready_rows_env(monkeypatch):
    import duckdb
    from duckdb.datasource import read_datasource

    monkeypatch.setenv("VANE_VIDEO_SOURCE_UDF_BACKEND", "ray_task")
    monkeypatch.setenv("VANE_VIDEO_SOURCE_UDF_MAX_READY_ROWS", "64")
    con = duckdb.connect()
...
def test_video_frame_source_map_batches_is_streaming_generator():
    assert inspect.isgeneratorfunction(_video_frame_source_map_batches)
