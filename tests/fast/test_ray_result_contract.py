...
def test_query_driver_copy_progress_contract_failure_still_tears_down(monkeypatch):
    cls, runner = _make_local_query_driver_actor()
    logical_plan = _FakeLogicalPlan(
        _FakePhysicalPlanWithoutPlanAttr("copy-progress-contract-failure")
    )
    teardown_calls = []
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (_ for _ in ()).throw(
            RuntimeError("invalid progress topology")
        ),
    )
    monkeypatch.setattr(
        cls,
        "_teardown_plan_resources",
        lambda _self, plan_id, query_id, *, drop_fragments: teardown_calls.append(
            (plan_id, query_id, drop_fragments)
        ),
    )
...
def test_query_driver_copy_accepts_plan_success_before_startup_barriers(monkeypatch):
    ...
    cls, runner = _make_local_query_driver_actor()
    logical_plan = _FakeLogicalPlan(
        _FakePhysicalPlanWithoutPlanAttr("copy-plan-before-startup-barriers")
    )
...
def test_query_driver_copy_plan_failure_interrupts_startup_barriers(monkeypatch):
    ...
    cls, runner = _make_local_query_driver_actor()
    logical_plan = _FakeLogicalPlan(
        _FakePhysicalPlanWithoutPlanAttr("copy-plan-startup-failure")
    )
...
def test_query_driver_copy_startup_barrier_failure_tears_down_plan(
    ...
    cls, runner = _make_local_query_driver_actor()
    logical_plan = _FakeLogicalPlan(
        _FakePhysicalPlanWithoutPlanAttr(f"copy-{failing_barrier}-barrier-failure")
    )
    teardown_started = threading.Event()
    marked_ready = []
...
def test_teardown_fence_failure_retains_retryable_query_ownership(monkeypatch):
    ...
    def fail_before_owner_release(_self, actual_query_id):
        raise driver.QueryTeardownOwnershipError(
            f"planned admission fence failure for {actual_query_id}"
        )
...
def test_describe_native_progress_materializes_deferred_clone_without_execution(tmp_path):
    ...
    con.execute(
        f"COPY (SELECT i::INTEGER AS i FROM range(10) tbl(i)) "
    ...
    assert any(
        "TABLE_SCAN" in pipeline["operators"]
        for pipeline in topology["pipelines"]
    )
    assert all(
        set(pipeline)
        == {"pipeline_id", "operators", "operator_details", "stage_ids"}
        for pipeline in topology["pipelines"]
    )
    ...
    assert [
        (pipeline["pipeline_id"], pipeline["operators"])
    ...
    scan_pipeline = next(
        pipeline for pipeline in final_pipelines if "TABLE_SCAN" in pipeline["operators"]
    )
...
def test_remote_exchange_sink_progress_does_not_add_result_collector(tmp_path, monkeypatch):
    ...
    for topology, result in zip(sink_topologies, sink_results, strict=True):
        ...
        assert all(
            pipeline["total_pipeline_tasks"] > 0
            and pipeline["completed_pipeline_tasks"]
            == pipeline["total_pipeline_tasks"]
            and pipeline["queued_pipeline_tasks"] == 0
            and pipeline["running_pipeline_tasks"] == 0
            for pipeline in final_pipelines
        )
    assert all(
        pipeline["operators"] != ["EXCHANGE_SINK"]
        for topology in sink_topologies
        for pipeline in topology["pipelines"]
    )
...
def test_run_plan_rejects_old_psets_argument_shape():
    runner = duckdb.ray_cxx.DistributedPhysicalPlanRunner()
    plan = _make_test_physical_plan()

    with pytest.raises(TypeError):
        runner.run_plan(plan, {"11": [object()]}, None)


def test_run_copy_plan_rejects_old_psets_argument_shape():
    runner = duckdb.ray_cxx.DistributedPhysicalPlanRunner()
    plan = _make_test_physical_plan()

    with pytest.raises(TypeError):
        runner.run_copy_plan(plan, {"11": [object()]}, None)
