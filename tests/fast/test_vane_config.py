...
def test_configure_rejects_native_fte_runner(monkeypatch):
    monkeypatch.delenv("VANE_RUNNER", raising=False)

    with pytest.raises(ValueError, match="runner must be 'local' or 'ray'"):
        configure(runner="native-fte")


def test_configure_rejects_old_native_runner_name():
    with pytest.raises(ValueError, match="runner must be 'local' or 'ray'"):
        configure(runner="native")
...
def test_get_or_infer_runner_type_rejects_native_alias():
    script = """
import os
import duckdb.runners as runners
...
try:
    ...
    assert "Invalid runner type" in str(exc)
else:
    raise AssertionError("native runner alias must be rejected")
"""
    subprocess.run([sys.executable, "-c", script], check=True)
...
def test_config_registry_contains_stable_public_fields():
    ...
    assert "native_runner_batch_size" not in fields
    assert "local_runner_batch_size" not in fields
