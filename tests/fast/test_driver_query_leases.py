...
def _allocation() -> QueryAllocation:
    ...
    return QueryAllocation(
        resources=resources,
        node_allocations=(
            NodeResourceAllocation(node_id="node-a", resources=resources),
        ),
        actor_placements=(),
        generation=1,
    )
...
def test_driver_task_lease_wait_is_event_driven_and_release_wakes_next_request():
    async def scenario():
        ...
        second_waiter = asyncio.create_task(
            runner_cls.acquire_query_task_lease(runner, second_request)
        )
        for _ in range(3):
        ...
        second_state = runner._query_task_lease_requests[
...
def test_driver_resource_change_evaluates_only_the_selected_task_waiter():
    async def scenario():
        ...
        pending_requests = [
            _task_request(query_id, f"request:pending:{index}", f"task:pending:{index}")
            for index in range(6)
        ]
        pending_tasks = [
            asyncio.create_task(runner_cls.acquire_query_task_lease(runner, request))
...
def test_driver_resource_change_event_drives_fte_owner_without_polling(monkeypatch):
    ...
    async def scenario():
        ...
        for _ in range(20):
            await asyncio.sleep(0)
            task = runner._query_fte_admission_pumps.get(
                "query-fte-resource-wake"
            )
...
def test_driver_pending_task_lease_can_be_cancelled_without_a_polling_wakeup():
    async def scenario():
        ...
        pending = asyncio.create_task(
            runner_cls.acquire_query_task_lease(runner, pending_request)
        )
        for _ in range(3):
            await asyncio.sleep(0)
...
def test_driver_pending_output_waiter_is_live_and_cancel_cleanup_is_complete():
    async def scenario():
        ...
        pending = asyncio.create_task(
            runner_cls.acquire_query_output_block_lease(runner, output_request)
        )
        for _ in range(3):
            await asyncio.sleep(0)
        ...
        assert set(runner._query_output_lease_requests) == {
            request["request_id"] for request, _grant in owned
        }
...
def test_driver_output_pump_evaluates_each_waiter_once_per_state_transition():
    async def scenario():
        ...
        output_tasks = [
            asyncio.create_task(
                runner_cls.acquire_query_output_block_lease(runner, request)
            )
...
def test_task_attempt_rejects_a_second_request_id_instead_of_orphaning_future():
    async def scenario():
        ...
        first_waiter = asyncio.create_task(
            runner_cls.acquire_query_task_lease(runner, first_request)
        )
        for _ in range(4):
            await asyncio.sleep(0)
...
def test_output_block_rejects_a_second_request_id_instead_of_orphaning_future():
    async def scenario():
        ...
        first_waiter = asyncio.create_task(
            runner_cls.acquire_query_output_block_lease(runner, first_request)
        )
        for _ in range(4):
            await asyncio.sleep(0)
        assert not first_waiter.done()
...
def test_cancelled_admission_request_replay_returns_the_same_terminal_denial():
    async def scenario():
        ...
        original_waiter = asyncio.create_task(
            runner_cls.acquire_query_task_lease(runner, cancelled_request)
        )
        for _ in range(4):
            await asyncio.sleep(0)
...
def test_query_teardown_resolves_all_pending_admission_futures():
    ...
    async def scenario():
        ...
        pending_task = asyncio.create_task(
            runner_cls.acquire_query_task_lease(runner, pending_task_request)
        )
...
def test_fragment_drop_waits_for_fte_admission_pump_before_registry_drop(monkeypatch):
    ...
    async def scenario():
        ...
        task = asyncio.create_task(
            runner_cls._run_query_fte_admission_pump(runner, query_id, done)
        )
...
def test_fragment_drop_keeps_query_resources_when_local_fte_registry_cannot_quiesce(monkeypatch):
    ...
    async def scenario():
        ...
        monkeypatch.setattr(
            ...
            lambda _query_id: (_ for _ in ()).throw(
                RuntimeError("watcher still alive")
            ),
        )
...
def test_background_query_teardown_fences_new_admission_before_table_purge():
    ...
    async def scenario():
        ...
        late_waiter = asyncio.create_task(
...
def test_task_admission_pump_resolves_waiter_when_stage_becomes_terminal():
    async def scenario():
        ...
        assert await runner_cls.acquire_query_task_lease(
            runner,
            pending_request,
        ) == denial
        assert await runner_cls.release_query_task_lease(
