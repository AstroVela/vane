...
class _FakeRemoteMethod:
    ...
    def remote(self, *args, **kwargs):
        resolved_args = [
            arg.future().result() if isinstance(arg, _ImmediateObjectRef) else arg
            for arg in args
        ]
...
def _register_test_query_graph(query_id, fragment_ids, *, max_concurrency=256):
    ...
    fragment_ids.update(
        f"{query_id}:node:{node_id}"
        for node_id in ("scan", "exchange", "upstream-worker", "worker-retry")
    )
    stages = tuple(
        StageResourceSpec(
            query_id=query_id,
...
def test_fragment_plan_ref_cache_is_session_scoped(monkeypatch):
    ...
    ref_a = worker_handle_mod._fragment_plan_ref(
        "query-cache", "query-cache:node:7", {"plan": "a"}
    )
    ref_a_again = worker_handle_mod._fragment_plan_ref(
        "query-cache", "query-cache:node:7", {"plan": "ignored"}
    )
    session = "session-b"
    ref_b = worker_handle_mod._fragment_plan_ref(
        "query-cache", "query-cache:node:7", {"plan": "b"}
    )
...
def test_fragment_plan_cache_drop_uses_exact_query_ownership(monkeypatch):
    ...
    query_ref = worker_handle_mod._fragment_plan_ref(
        "q", "q:node:1", {"plan": "q"}
    )
    child_ref = worker_handle_mod._fragment_plan_ref(
        "q:child", "q:child:node:1", {"plan": "child"}
    )
...
def test_fte_worker_actor_handle_wraps_control_rpcs():
    ...
    handle._fragment_query_ids.update(
        {"q:node:a": "q", "other:node:b": "other"}
    )
...
def test_create_task_timeout_retains_remote_mutation_ownership(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            TimeoutError("planned create timeout")
        ),
    )

    with pytest.raises(TimeoutError, match="did not complete"):
        handle.fte_create_task(
            {"task_id": task_id, "fragment_id": f"{query_id}:node:1"}
        )
...
def test_remote_drop_timeout_retains_fence_and_local_generation(monkeypatch):
    ...
    with worker_handle_mod._FRAGMENT_PLAN_REF_CACHE_LOCK:
        worker_handle_mod._FRAGMENT_PLAN_REF_CACHE[
            ("drop-timeout-test-session", query_id, fragment_id)
        ] = object()
    monkeypatch.setattr(
        task_control_mod,
        "resolve_object_refs_blocking",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            TimeoutError("planned remote drop timeout")
        ),
    )
    ...
    assert (
        worker_handle_mod._FTE_ACTIVE_TEARDOWN_OPERATIONS_BY_QUERY[query_id]
        == 1
    )
    ...
    future.complete(
        {"tasks_removed": 0, "tasks_canceled": 0, "fragments_removed": 0}
    )
...
def test_pending_teardown_on_one_worker_does_not_block_drop_fanout(monkeypatch):
    ...
    assert [call for call in actor2.fte_calls if call[0] == "drop_query"] == [
        ("drop_query", query_id)
    ]
...
def test_control_future_completed_during_zero_timeout_probe_is_reloaded(monkeypatch):
    ...
    monkeypatch.setattr(
        task_control_mod,
        "resolve_object_refs_blocking",
        lambda refs, **_kwargs: (_ for _ in ()).throw(
            TimeoutError("bulk barrier timed out")
        )
        if isinstance(refs, list)
...
def test_fte_worker_actor_handle_async_control_retries_submission_failure(monkeypatch):
    ...
    monkeypatch.setattr(worker_handle_mod.ray, "get", lambda value, *_args, **_kwargs: {"state": "RUNNING", "ref": value})
...
def test_fte_control_ref_uses_async_actor_safe_get(monkeypatch):
    ...
    assert task_control_mod.FteWorkerTaskControlMixin._get_fte_control_ref(
        "fte_get_task_status",
        "status-ref",
        timeout_s=7.5,
    ) == "resolved"
...
def test_fte_dynamic_exchange_running_window_defers_extra_partitions(monkeypatch):
    ...
    assert get_query_resource_manager("query-dynamic-window").snapshot()["stages"][stage_id][
        "pending_task_count"
    ] == 0
    assert [call[1]["task_id"]["partition_id"] for call in actor.fte_calls if call[0] == "create"] == [0]
...
def test_fte_submission_window_keeps_36_descriptors_but_only_7_in_qrm(monkeypatch):
    ...
    assert stats["stage_submission_block_count"] == 1
    assert sum(
        partition.execution_ready_deferred
...
def test_fte_pending_drain_is_fair_across_queries(monkeypatch):
    ...
    assert [str(task_handle.task_id) for task_handle in handle.pop_fte_result_handles("query-a")] == [
        "query-a.0.0.0"
    ]
...
def test_fte_pending_drain_prefers_standard_over_speculative(monkeypatch):
    ...
    assert [
        str(task_handle.task_id) for task_handle in handle.pop_fte_result_handles("query-running-standard")
...
def test_fte_immediate_speculative_waits_behind_standard_pending(monkeypatch):
    ...
    assert speculative_pending == []
    assert [
        str(task_handle.task_id) for task_handle in handle0.pop_fte_result_handles("query-immediate-running")
    ...
    scheduled = handle1.pop_fte_result_handles(
        "query-immediate-standard-pending"
    ) + handle1.pop_fte_result_handles("query-immediate-auto-speculative")
...
def test_fte_eager_speculative_drains_before_standard_without_blocking_it(monkeypatch):
    ...
    assert [str(task_handle.task_id) for task_handle in standard_scheduled] == [
        "query-pending-after-eager.0.0.0"
    ]
...
def test_fte_pending_execution_class_transition_does_not_bypass_hard_capacity(monkeypatch):
    ...
    assert pending == []
    assert [
        str(task_handle.task_id) for task_handle in handle.pop_fte_result_handles("query-transition-blocker")
...
def test_fte_worker_failure_retry_preserves_registered_heap(monkeypatch):
    ...
    assert (
        worker_handle_mod._FTE_PARTITION_OWNERS[("query-memory-retry", "query-memory-retry:node:7", 0)]
        is low_memory
    )
...
def test_fte_worker_failure_retry_waits_for_scheduling_delayer(monkeypatch):
    ...
    assert len(first) == 1
    assert [
        str(task_handle.task_id) for task_handle in handle0.pop_fte_result_handles("query-fte-retry-delay")
...
def test_fte_split_append_control_failure_replays_on_replacement(monkeypatch):
    ...
    assert "worker-0" not in worker_handle_mod._FTE_WORKER_HANDLES
    assert (
        worker_handle_mod._FTE_PARTITION_OWNERS[("query-fte-append-lost", "query-fte-append-lost:node:7", 0)]
        is handle1
    )
    assert retries[0].worker_handle is handle1
    retry_creates = [call for call in actor1.fte_calls if call[0] == "create"]
...
def test_fte_split_queue_full_replays_descriptor_on_replacement(monkeypatch):
    ...
    assert "worker-0" not in worker_handle_mod._FTE_WORKER_HANDLES
    assert (
        worker_handle_mod._FTE_PARTITION_OWNERS[("query-fte-queue-full", "query-fte-queue-full:node:7", 0)]
        is handle1
    )
...
def test_fte_worker_failure_replays_all_owned_stage_partitions(monkeypatch):
    ...
    assert (
        worker_handle_mod._FTE_PARTITION_OWNERS[("query-host-loss", "query-host-loss:node:exchange", 0)] is handle1
    )
    assert handle0.fte_pressure_stats()["running_attempt_count"] == 0
...
def test_fte_exchange_selector_event_updates_running_and_pending_consumers(monkeypatch):
    ...
    stage = worker_handle_mod._FTE_FRAGMENT_EXECUTIONS[
        ("query-fte-selector-mixed", "query-fte-selector-mixed:node:8")
    ]
...
def test_fte_exchange_selector_version_stale_update_is_ignored(monkeypatch):
    ...
    selector_stats = handle.fte_registry_stats()["queries"]["query-fte-selector-versioned"][
        "fragment_executions"
    ]["query-fte-selector-versioned:node:8"]["exchange_selectors"]["3"]
...
def test_fte_exchange_selector_materializes_preselected_partition(monkeypatch):
    ...
    assert add_call[2] == "3"
    assert [split["source_partition_id"] for split in add_call[3]] == [1]
    selector_stats = handle.fte_registry_stats()["queries"]["query-fte-selector-materialize"][
        "fragment_executions"
    ]["query-fte-selector-materialize:node:8"]["exchange_selectors"]["3"]
...
def test_fte_exchange_selector_final_requires_full_coverage(monkeypatch):
    ...
    selector_stats = handle.fte_registry_stats()["queries"]["query-fte-selector-final-coverage"][
        "fragment_executions"
    ]["query-fte-selector-final-coverage:node:8"]["exchange_selectors"]["3"]
...
def test_fte_exchange_selector_final_version_replay_is_idempotent(monkeypatch):
    ...
    selector_stats = handle.fte_registry_stats()["queries"]["query-fte-selector-final-version"][
        "fragment_executions"
    ]["query-fte-selector-final-version:node:8"]["exchange_selectors"]["3"]
    assert selector_stats["version"] == 1
...
def test_worker_pressure_drop_uses_exact_query_identity():
    ...
    child_reservation = partition_reservation_key(
        "q|child", "q|child:node:1", 0
    )
    pressure.running_attempts.update({parent_attempt, child_attempt})
    pressure.split_counts_by_attempt.update(
        {parent_attempt: 1, child_attempt: 2}
    )
    pressure.reserved_partitions.update(
        {parent_reservation, child_reservation}
    )
    pressure.memory_bytes_by_reservation.update(
        {parent_reservation: 10, child_reservation: 20}
    )
...
def test_fragment_registration_ownership_lives_until_remote_actor_completion():
    ...
    actor = _FakeActor()
    actor.register_fragments = _DeferredRemoteMethod(
        _DeferredObjectRef(deferred_future)
    )
    handle = RayWorkerActorHandle(actor, memory_capacity_bytes=1 << 60)
    task = _FakeTask(
        name="scan-task",
...
def test_fragment_registration_cleanup_uses_exact_query_ownership():
    ...
    with handle._fragment_registration_lock:
        handle._registered_fragment_ids.update(
            {parent_fragment, child_fragment}
        )
        handle._fragment_query_ids.update(
            {parent_fragment: "q", child_fragment: "q:child"}
        )
