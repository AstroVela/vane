...
def test_async_actor_execution_mode_is_removed():
    from duckdb.execution.udf_ray import RemoteUDFExecutor, UDFActorPool

    payload = _payload(async_mode=True)
    pool = UDFActorPool._from_handles([_Actor()], payload=payload, actor_dispatch_indices={0})
    with pytest.raises(ValueError, match="async_mode has been removed"):
        RemoteUDFExecutor(pool, payload)
