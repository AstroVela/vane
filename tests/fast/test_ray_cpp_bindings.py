...
def test_distributed_physical_plan_inspectors():
    ...
    assert not hasattr(plan, "scan_task_map")


def test_distributed_physical_plan_runner_run_plan_rejects_old_psets_argument_shape():
    m = duckdb.ray_cxx
    runner = m.DistributedPhysicalPlanRunner()
    plan = _make_test_physical_plan()

    with pytest.raises(TypeError):
        runner.run_plan(plan, {"pset": [1, 2, None]}, None)
