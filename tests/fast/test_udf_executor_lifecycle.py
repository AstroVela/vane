...
def test_streaming_wakeups_use_owned_joined_dispatcher_queue():
    source = (
        Path(__file__).resolve().parents[2] / "external/duckdb/src/execution/operator/projection/physical_udf_inout.cpp"
    ).read_text()
    start = source.index("static bool RunStreamingWakeCallbacksUnlocked")
    end = source.index("static bool BlockStreamingTask", start)
    wakeup_implementation = source[start:end]

    assert "EnqueueDeferredWakeup" in wakeup_implementation
    assert ".detach()" not in wakeup_implementation

    dispatcher_source = (Path(__file__).resolve().parents[2] / "src/duckdb_py/udf_executor.cpp").read_text()
    assert "DrainDeferredWakeups" in dispatcher_source
    assert "dispatcher_thread.join()" in dispatcher_source


def test_non_streaming_lazy_output_handoffs_lease_to_descriptor_ownership():
    executor_header = (
        Path(__file__).resolve().parents[2] / "external/duckdb/src/include/duckdb/execution/udf_executor.hpp"
    ).read_text()
    assert "struct UDFResult" in executor_header
    udf_result = executor_header.split("struct UDFResult", 1)[1].split("};", 1)[0]
    assert "handoff_output_lease" in udf_result

    operator_source = (
        Path(__file__).resolve().parents[2] / "external/duckdb/src/execution/operator/projection/physical_udf_inout.cpp"
    ).read_text()
    result_handler = operator_source.split("static bool TakeReadyResultOnce", 1)[1].split(
        "static bool DrainReadyResult", 1
    )[0]
    assert "handoff_output_lease" in result_handler
    assert "ownership_tokens.push_back" in result_handler


def test_subprocess_startup_shutdown_and_warmup_are_strict_and_bounded():
    subprocess_source = (Path(__file__).resolve().parents[2] / "duckdb/execution/udf_subprocess.py").read_text()
    worker_source = (Path(__file__).resolve().parents[2] / "duckdb/execution/udf_subprocess_worker.py").read_text()

    startup = subprocess_source.split("def _start_worker", 1)[1].split("def _recv_expected", 1)[0]
    assert "timeout_s=_subprocess_control_timeout_s()" in startup
    assert "proc.wait(timeout=" in subprocess_source
    assert "except AttributeError" not in worker_source
...
def test_dispatcher_unregister_has_one_bounded_wait_per_phase():
    source = (Path(__file__).resolve().parents[2] / "src/duckdb_py/udf_executor.cpp").read_text()
    unregister = source.split("void Unregister", 1)[1].split("void NotifyWork", 1)[0]

    assert "while (!slot->cleanup_complete" not in unregister
    assert "while (!async_shutdown_complete" not in unregister
    assert unregister.count("wait_for(") == 2


def test_dispatcher_terminal_events_cannot_underflow_or_leak_output_lease():
    source = (Path(__file__).resolve().parents[2] / "src/duckdb_py/udf_executor.cpp").read_text()
    release_terminal = source.split("void ReleaseTerminalSubmit", 1)[1].split("void QueueTerminalControlResult", 1)[0]
    deliver = source.split("bool DeliverOutputEvent", 1)[1].split("bool TryDeliverResult", 1)[0]

    assert "terminal_submit_ids.insert" in release_terminal
    assert "inflight_before <= 0" in release_terminal
    assert "release_on_consumer_error" in deliver
...
def test_udf_runtime_has_no_legacy_payload_or_retry_fallbacks():
    root = Path(__file__).resolve().parents[2]
    ray_source = (root / "duckdb/execution/udf_ray.py").read_text()
    runtime_source = (root / "duckdb/execution/_udf_runtime.py").read_text()
    scalar_source = (root / "external/duckdb/src/function/scalar/udf.cpp").read_text()
    runner_source = (root / "external/duckdb/src/include/duckdb/function/scalar/udf_functions.hpp").read_text()
    connection_source = (root / "src/duckdb_py/pyconnection.cpp").read_text()
    connection_header = (root / "src/duckdb_py/include/duckdb_python/pyconnection/pyconnection.hpp").read_text()

    assert 'options.get("max_task_retries") or' not in ray_source
    assert "self._on_error" not in runtime_source
    assert "self._max_retries" not in runtime_source
    assert "Legacy non-expression UDF payloads" not in scalar_source
    assert 'runner_type == "native"' not in runner_source
    assert "ResolveVaneSQLExecutionBackend" not in connection_source
    create_vane_signature = connection_header.split("CreateVaneFunctionInternal", 1)[1].split(";", 1)[0]
    assert "execution_backend" not in create_vane_signature


def test_dead_python_udf_operator_chain_and_physical_compatibility_slots_are_removed():
    root = Path(__file__).resolve().parents[2] / "external/duckdb/src"
    removed_paths = (
        "function/scalar/python_udf.cpp",
        "include/duckdb/function/scalar/python_udf_functions.hpp",
        "optimizer/python_udf_project_rewriter.cpp",
        "include/duckdb/optimizer/python_udf_project_rewriter.hpp",
        "planner/operator/logical_python_udf_project.cpp",
        "include/duckdb/planner/operator/logical_python_udf_project.hpp",
        "execution/physical_plan/plan_python_udf_project.cpp",
    )
    ...
    sources = "\n".join(
        path.read_text()
        for path in (
            root / "function/function_list.cpp",
            root / "optimizer/optimizer.cpp",
            root / "execution/physical_operator.cpp",
            root / "execution/distributed/pipeline_node/translate.cpp",
            root / "include/duckdb/common/enums/logical_operator_type.hpp",
            root / "include/duckdb/common/enums/physical_operator_type.hpp",
        )
    )
    assert "PythonUDFFunction" not in sources
    assert "PythonUDFProjectRewriter" not in sources
    assert "LOGICAL_PYTHON_UDF_PROJECT" not in sources
    assert "PhysicalOperatorType::UDF_PROJECT" not in sources
    assert "PhysicalOperatorType::PYTHON_UDF_PROJECT" not in sources


def test_unified_python_executor_contract_has_no_optional_wakeup_or_close_fallback():
    root = Path(__file__).resolve().parents[2]
    interface_source = (root / "duckdb/execution/unified_executor.py").read_text()
    dispatcher_source = (root / "src/duckdb_py/udf_executor.cpp").read_text()
    ray_source = (root / "duckdb/execution/udf_ray.py").read_text()

    assert "supports_async_wakeup" not in interface_source
    assert "def register_wakeup" in interface_source
    assert "def close" in interface_source
    assert 'py::hasattr(slot_ptr->py_executor->obj, "close")' not in dispatcher_source
    assert 'py::hasattr(slot.py_executor->obj, "shutdown")' not in dispatcher_source
    assert 'attr("supports_async_wakeup")' not in dispatcher_source
    assert ray_source.count("def close(self) -> None:") >= 2

    cpp_interface = (root / "external/duckdb/src/include/duckdb/execution/udf_executor.hpp").read_text()
    udf_executor = cpp_interface.split("class UDFExecutor", 1)[1].split("using udf_executor_factory_t", 1)[0]
    assert "UDF executor does not support" not in udf_executor
    assert "return UDFWakeupRegistrationResult::UNSUPPORTED" not in udf_executor
    assert "return InsertionOrderPreservingMap<string>()" not in udf_executor


def test_ray_driver_result_delivery_does_not_poll_object_refs_or_task_completion():
    source = (Path(__file__).resolve().parents[2] / "duckdb/runners/ray/driver.py").read_text()
    assert "ray.wait(" not in source
    get_result = source.split("async def get_result(self)", 1)[1].split("def cancel(self)", 1)[0]
    assert "while not self.done()" not in get_result
    assert "asyncio.sleep" not in get_result
    status_watcher = source.split("async def _watch_status(self)", 1)[1].split("async def _publish_worker_failure", 1)[
        0
    ]
    assert "poll_interval_s" not in status_watcher
    assert "asyncio.sleep" not in status_watcher
...
def test_udf_executor_is_the_only_base():
    from duckdb.execution.udf_ray import RayTaskUDFExecutor
    from duckdb.execution.udf_subprocess import UDFExecutor as SubprocessUDFExecutor
    from duckdb.execution.unified_executor import UDFExecutor

    assert issubclass(SubprocessUDFExecutor, UDFExecutor)
    assert issubclass(RayTaskUDFExecutor, UDFExecutor)


def test_current_executor_modules_remain_importable():
    for module_name in (
        "duckdb.execution.udf",
        "duckdb.execution.udf_subprocess",
...
def test_legacy_function_executor_modules_are_removed():
    for module_name in (
        "duckdb.execution.udf_local",
        "duckdb.execution.function_udf",
        "duckdb.execution.function_udf_local",
        "duckdb.execution.function_udf_ray",
    ):
        sys.modules.pop(module_name, None)
...
def test_unified_executor_rejects_legacy_routing_payload():
    from duckdb.execution.unified_executor import build_unified_executor

    with pytest.raises(ValueError, match="payload.call_mode"):
        build_unified_executor({"function_pickle": b"not-used", "use_ray": True})
...
def test_subprocess_submit_table_uses_local_shm_descriptor_input():
    import duckdb.execution.udf_subprocess as subprocess_exec

    assert not hasattr(subprocess_exec, "_arrow_table_to_ipc_bytes")
...
def test_subprocess_task_worker_slots_control_pool_size(monkeypatch):
    ...
    try:
        ...
        assert not hasattr(executor, "_delegate")
        assert executor._executor is None
...
def test_subprocess_actor_ref_bundle_output_uses_async_single_worker_path():
    import duckdb.execution.udf_subprocess as subprocess_exec
...
def test_subprocess_actor_direct_output_uses_async_single_worker_path():
    import duckdb.execution.udf_subprocess as subprocess_exec
...
def test_ray_task_direct_submit_path_is_removed():
    executor, run_stream, run_ref_stream = _ray_task_executor()

    with pytest.raises(RuntimeError, match="direct submit path has been removed"):
        executor.submit(pa.table({"x": [1]}))
    with pytest.raises(RuntimeError, match="direct ref-bundle submit path has been removed"):
        executor.submit_ref_bundle([], [], [], [])

    assert run_stream.calls == []
    assert run_ref_stream.calls == []
    assert executor.take_ready_result() is None
...
def test_local_shm_descriptor_wrap_balances_resource_tracking_without_global_monkeypatch(monkeypatch):
    ...
    source = (Path(__file__).resolve().parents[2] / "duckdb/execution/ref_bundle.py").read_text()
    assert "_with_shared_memory_resource_tracker_suppressed" not in source
    assert "_resource_tracker.register =" not in source
    assert "_resource_tracker.unregister =" not in source
