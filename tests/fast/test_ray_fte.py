...
def test_fte_partition_admission_uses_non_persistent_descriptor_arbitration(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: submission_events.append(
            ("resolve", args, kwargs)
        ),
    )
    monkeypatch.setattr(
...
def test_internal_fte_query_uses_outer_query_resource_identity():
    ...
    worker = _PlacementWorker()
    placement = fte_fragment_scheduler.FteWorkerPlacementManager(
        _PlacementCoordinator(worker)
    )
    try:
        ...
        assert reservation.stage_id == stage_id
        assert manager.snapshot()["task_leases"][reservation.task_lease_id][
            "query_id"
        ] == resource_query_id
        assert payload["query_id"] == resource_query_id
        assert payload["execution_query_id"] == execution_query_id
        assert fte_fragment_scheduler.fte_execution_query_ids_for_resource(
            resource_query_id
        ) == (execution_query_id,)
    finally:
        fte_fragment_scheduler._drop_fte_registry_for_query(
            execution_query_id
        )
        clear_query_resource_managers()
...
def test_fte_query_drop_releases_all_task_leases():
    ...
    try:
        ...
        assert released == 2
        assert manager.snapshot()["task_leases"] == {}
        assert sorted(worker.released) == sorted(
            [(query_id, fragment_id, 0), (query_id, fragment_id, 1)]
        )
    finally:
        _cleanup_fte_query(query_id)
    ...
    def _collect() -> None:
        try:
            if snapshot_kind == "progress":
                snapshot = fte_fragment_scheduler.fte_progress_registry_snapshot(
                    query_id
                )
            else:
...
def test_registry_snapshot_is_observation_only(monkeypatch, snapshot_kind):
    ...
    monkeypatch.setattr(
        ...
        lambda _fragment: (_ for _ in ()).throw(
            AssertionError("progress collection must not mutate scheduler state")
        ),
    )
    try:
        if snapshot_kind == "progress":
    ...
    try:
        ...
        assert not fte_fragment_scheduler.ensure_fte_fragment_progress_topology(
            ...
            lambda: (_ for _ in ()).throw(
                AssertionError("published topology must not be rebuilt")
            ),
        )
        assert build_count == 1
    finally:
...
def test_fte_fragment_execution_does_not_probe_a_sync_control_fallback():
    ...
    assert [command.command_type for command in result.worker_commands] == [
        "FteAddSplitsCommand"
    ]
...
def test_fte_worker_task_manager_cancel_finished_loser_releases_result_immediately():
    ...
    async def run():
        ...
        task_id = "q.0.11.1"
        initial = await manager.create_task(
            {"task_id": task_id, "fragment_id": "q:node:scan"}
        )
...
def test_ray_fte_worker_requires_exact_query_lease_heap():
    ...
    async def run():
        ...
        with pytest.raises(RuntimeError, match="requires query_task_lease"):
            await manager.create_task(
                {"task_id": task_id, "fragment_id": "qlease:node:scan"}
            )
