...
def _run_subprocess_task_direct_compute_batch_probe(tmp_path):
    ...
    env = dict(os.environ)
    result = subprocess.run([sys.executable, "-c", script], env=env, capture_output=True, text=True, timeout=30, check=True)
    log = result.stdout + result.stderr
    for line in reversed(log.splitlines()):
        if line.startswith("VANE_BATCH_COUNTS="):
            return {int(key): value for key, value in json.loads(line.split("=", 1)[1]).items()}
...
def test_map_batches_nested_reserved_field_name_round_trips_at_bind():
    ...
    def make_features(table):
        ...
        return pa.table(
            {"features": pa.array(values, type=feature_arrow_type)}
        )

    feature_type = duckdb.sqltype(
        'STRUCT("label" BIGINT, confidence DOUBLE, bbox DOUBLE[])'
    )
    rel = duckdb.connect().sql("select i::BIGINT AS x from range(2) t(i)").map_batches(
        make_features,
...
def test_streaming_breaker_waits_for_tail_events_without_source_finalize_spin():
    ...
    source_calls = [
        int(value) for value in re.findall(r"source_calls=(\d+)", result.stderr)
    ]
    assert source_calls
...
def test_map_batches_streaming_output_splits_by_actual_bytes_without_row_preserving(tmp_path):
    ...
    con = duckdb.connect()
    rel = con.sql("select i::BIGINT as x from range(8) t(i)").map_batches(
        make_tensor,
