...
def test_install_non_existent_extension():
    ...
    with pytest.raises(vane.IOException) as exc:
        conn.install_extension("non-existent")

    if not isinstance(exc, vane.HTTPException):
...
def test_install_misuse_errors(duckdb_cursor):
    ...
    with pytest.raises(
        vane.InvalidInputException, match="The provided 'repository' or 'repository_url' can not be empty!"
    ):
    ...
    with pytest.raises(vane.InvalidInputException, match="The provided 'version' can not be empty!"):
