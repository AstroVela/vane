...
from vane.runners.fte.fte_split_assigner import _normalize_sources
from vane.runners.fte.fte_types import FteSplit, FteTaskAttemptId, FteTaskId
...
def normalize_initial_splits(
    initial_splits: Mapping[str, Sequence[FteSplit | Mapping[str, Any]]] | None,
...
class FteTaskUpdateRequest:
    ...
    def from_dict(cls, update: Mapping[str, Any]) -> FteTaskUpdateRequest:
        ...
        return cls(
            ...
            source_node_ids=_normalize_sources(payload.get("source_node_ids")),
            dynamic_scan_source_node_ids=_normalize_sources(payload.get("dynamic_scan_source_node_ids")),
            dynamic_exchange_source_node_ids=_normalize_sources(payload.get("dynamic_exchange_source_node_ids")),
        )
...
class TaskDescriptor:
    ...
    def append_splits(
        self,
        source_node_id: str,
        splits: Sequence[FteSplit | Mapping[str, Any]],
    ) -> list[FteSplit]:
    ...
    def apply_task_update(self, update: FteTaskUpdateRequest | Mapping[str, Any] | None) -> bool:
        ...
        for attr in (
            ...
            merged_sources = set(current) | set(getattr(update_request, attr))
            if merged_sources != current:
                setattr(self, attr, merged_sources)
                changed = True
        if changed:
