...
class RayQueryResourceManager:
    ...
    def _unit_dimension_maximum_locked(
        ...
        for resource_field in _RESOURCE_FIELDS:
            per_task = _resource_dimension(commitment, resource_field)
            if per_task <= 0:
                continue
            limit = _resource_dimension(self.allocation.resources, resource_field)
        ...
        if math.isinf(coexistence_bound):
            return _resource_dimension(self.allocation.resources, field_name)
