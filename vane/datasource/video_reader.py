...
def _wait_for_memory() -> None:
    ...
    def has_capacity(mem: Any) -> bool:
        if _MEM_MIN_AVAILABLE_BYTES > 0 and mem.available >= _MEM_MIN_AVAILABLE_BYTES:
            return True
        return bool(mem.percent < _MEM_HIGH_WATERMARK)

    def has_recovered(mem: Any) -> bool:
        if _MEM_MIN_AVAILABLE_BYTES > 0 and mem.available >= _MEM_MIN_AVAILABLE_BYTES:
            return True
        return bool(mem.percent < _MEM_LOW_WATERMARK)
...
def _decord_frame_asnumpy(frame: Any, *, video_path: str) -> npt.NDArray[np.uint8]:
    try:
        return cast("npt.NDArray[np.uint8]", frame.asnumpy())
    except Exception as exc:
    ...
    def to_udf_relation(self, con: Any) -> Any:
        ...
        udf_kwargs = _video_source_udf_kwargs(
            ...
            schema={
                ...
                "frame": vane.tensor_type(vane.sqltypes.UTINYINT, (self.height, self.width, 3)),
            },
        )
