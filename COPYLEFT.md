...
When checking an extracted source tree without Git metadata, the filesystem
scan skips Python bytecode (`.pyc`, `.pyo`, and `$py.class`) generated by imports.
These files are excluded by the source-package builder. Other files inside
cache directories still receive the source check. Git-tracked files and
members of a distribution archive remain subject to the full inventory.
