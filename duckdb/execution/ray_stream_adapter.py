...
def validate_ray_stream_contract(ray_module: Any) -> None:
    ...
    missing = [
        name for name in _REQUIRED_GENERATOR_METHODS if not callable(getattr(object_ref_generator, name, None))
    ]
    if missing:
...
class RayStreamAdapter:
    ...
    def _validate_generator_if_started(self) -> None:
        ...
        missing = [
            name for name in _REQUIRED_GENERATOR_METHODS if not callable(getattr(self._generator, name, None))
        ]
        if missing:
            raise TypeError(
        ...
        if missing_core:
            raise TypeError(
                f"Ray {version!r} stream source is missing required CoreWorker contract: "
                + ", ".join(missing_core)
            )
