...
def execute_scalar_map_layout(
    ...
    if arg_count > table.num_columns:
        raise RuntimeError(
            "scalar_arg_count %d exceeds task input column count %d"
            % (arg_count, table.num_columns)
        )
    ...
    if output.num_rows != passthrough.num_rows:
        raise RuntimeError(
            "map task output rows %d do not match input rows %d"
            % (output.num_rows, passthrough.num_rows)
        )
    return pa.table(
