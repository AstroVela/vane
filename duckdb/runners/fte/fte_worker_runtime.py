    ...
    *,
    ...
    required_identity = {
        ...
        "execution_query_id": str(
            lease.get("execution_query_id") or ""
        ).strip(),
    ...
    if missing:
        raise RuntimeError(
            "Ray FTE query_task_lease is missing " + ", ".join(missing)
        )
    if required_identity["execution_query_id"] != task_id.query_id:
        raise RuntimeError(
            "Ray FTE query_task_lease execution_query_id does not match task"
        )
    ...
    explicit = (
        _memory_requirement_from_request(request, None)
        if _has_explicit_memory_requirement(request)
        else None
    )
    if explicit is not None and int(explicit) != heap_bytes:
        raise RuntimeError(
            "Ray FTE memory requirement diverges from query task lease: "
            f"request={explicit} lease={heap_bytes}"
        )
    return heap_bytes
...
class FteTaskExecution:
    ...
    def split_queue_status(self) -> dict[str, Any]:
        ...
        consumed_by_source = {
            source_id: int(queues_by_source[source_id].consumed_splits())
            for source_id in sorted(submitted_by_source)
        }
        completed_by_source = {
            source_id: int(queue.completed_splits())
            for source_id, queue in sorted(queues_by_source.items())
        }
        submitted_rows_by_source = {
            source_id: int(queue.submitted_rows())
            for source_id, queue in sorted(queues_by_source.items())
        }
        submitted_bytes_by_source = {
            source_id: int(queue.submitted_input_bytes())
            for source_id, queue in sorted(queues_by_source.items())
        }
        consumed_rows_by_source = {
            source_id: int(queue.consumed_rows())
            for source_id, queue in sorted(queues_by_source.items())
        }
        consumed_bytes_by_source = {
            source_id: int(queue.consumed_input_bytes())
            for source_id, queue in sorted(queues_by_source.items())
        }
        completed_rows_by_source = {
            source_id: int(queue.completed_rows())
            for source_id, queue in sorted(queues_by_source.items())
        }
        completed_bytes_by_source = {
            source_id: int(queue.completed_input_bytes())
            for source_id, queue in sorted(queues_by_source.items())
        }
...
class FteWorkerTaskManager:
        ...
        if execution is not None:
            ...
            parts.extend(
                [
                    ...
                    "queue_position="
                    f"{_format_admission_field(stats.get('executor_queue_position'))}",
    ...
    async def drop_query(self, query_id: str) -> dict[str, int]:
        ...
        if errors:
            details = "; ".join(
                f"task={key}: {type(exc).__name__}: {exc}"
                for key, exc in errors
            )
            raise RuntimeError(
                f"failed to fully drop FTE query {query_id}; retained failed task ownership: "
                + details
            ) from errors[0][1]
