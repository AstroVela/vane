...
def _native_runtime_info_fields(info: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_consumed_split_count_by_source": _debug_status_field(
            runtime_status, "consumed_split_count_by_source"
        ),
...
class NativeTaskResultHandle:
    def __init__(
        ...
        *,
        ...
        status_callback: Callable[[NativeTaskResultHandle, Mapping[str, Any], BaseException | None], None] | None = None,
    ) -> None:
        self._worker = worker
    ...
    def fragment_id(self) -> str:
        return str(self._request.get("fragment_id") or f"{self._task_id.query_id}:fragment-{self._task_id.fragment_execution_id}")
...
class _NativeFteProgressRegistry:
    ...
    def register_requests(self, requests: Sequence[Mapping[str, Any]]) -> None:
        with self._lock:
            for request in requests:
                ...
                query_id = task_id.query_id
                fragment_id = str(
                    request.get("fragment_id") or f"{query_id}:fragment-{task_id.fragment_execution_id}"
                )
        ...
        *,
        ...
        with self._lock:
            ...
            for fragment_id, registered in fragments.items():
                ...
                waiting_for_node_count = sum(1 for partition in partitions.values() if partition.get("waiting_for_node"))
                waiting_for_execution_count = sum(
                    1 for partition in partitions.values() if partition.get("waiting_for_execution")
                )
                deferred_count = sum(1 for partition in partitions.values() if partition.get("execution_ready_deferred"))
                ...
                fragment_executions[fragment_id] = {
                    ...
                    "progress_topology": copy.deepcopy(
                        registered.progress_topology
                        or {"schema": "pipeline_topology", "pipelines": []}
                    ),
        ...
        finished = bool(fragment_executions) and all(
            fragment["finished"] for fragment in fragment_executions.values()
        )
        return {
            "query_id": query_id,
    ...
    def _task_stats_from_partition_metrics(
        ...
        if running_attempts is not None:
            if not isinstance(running_attempts, Sequence) or isinstance(
                running_attempts, (str, bytes, bytearray)
            ):
    ...
    def _topology_from_task_stats(
        ...
        return validate_pipeline_topology(
            {"schema": "pipeline_topology", "pipelines": pipelines}
        )
    ...
    def _merge_fragment_progress_topology_locked(
        ...
        for task_stats in cls._task_stats_from_partition_metrics(metrics):
            ...
            if fragment.progress_topology is None:
                ...
                raise RuntimeError(
                    "native fragment progress topology changed after publication: "
...
class NativeFteWorkerManagerBackend:
    ...
    def fragment_stats_by_worker(self) -> dict[str, dict[str, int]]:
        ...
        for worker in self._workers:
            ...
            stats_by_worker[worker_id] = {
                key: _idx_stat(snapshot.get(key))
                for key in _FRAGMENT_STAT_KEYS
                if key in snapshot
            }
    ...
    def _partition_metrics_from_handle(
        ...
        *,
        ...
        initial_split_count_by_source = {}
        for key in ("submitted_split_count_by_source", "queued_split_count_by_source", "consumed_split_count_by_source"):
            value = status.get(key)
            if isinstance(value, Mapping):
        ...
        for handle in handles:
            ...
            if partition["state"] == "FAILED":
                failed_partitions.append(
                    {
                        "task_id": handle.fte_task_id(),
                        "latest_failure": repr(status_error) if status_error is not None else str(status.get("failure")),
                    }
                )
    ...
    def drop_query(self, query_id: str) -> None:
        ...
        try:
            ...
            worker_errors.append(
                f"progress_registry: {type(exc).__name__}: {exc}"
            )
        removed = 0
        canceled = 0
        for worker in self._workers:
        ...
        if worker_errors:
            raise RuntimeError(
                f"native FTE query teardown failed for {query_id}: "
                + "; ".join(worker_errors)
            )
    ...
    @staticmethod
    def materialize_task_context(request: Mapping[str, Any], *, merge_scan_task_descriptors: Callable[[list[Any]], Any]) -> dict[str, Any]:
