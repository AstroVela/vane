...
def validate_fte_status_identity(
    ...
    if not raw_identities:
        raise ValueError(
            f"FTE status identity missing for expected attempt {expected_attempt}"
        )
    for field_name, raw_identity in raw_identities:
        try:
            ...
            raise ValueError(
                f"FTE status identity is malformed in {field_name}: {raw_identity!r}"
            ) from exc
        if actual_attempt != expected_attempt:
            raise RuntimeError(
                "FTE status identity mismatch: "
                f"expected={expected_attempt} actual={actual_attempt} field={field_name}"
            )
