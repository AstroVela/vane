...
def _configure_duckdb_s3(conn: Any) -> None:
    ...
    # Explicitly clear http_proxy to prevent DuckDB from routing S3 requests
    # through a proxy that may not be reachable on worker nodes.
    conn.execute("SET GLOBAL http_proxy=''")
...
class RayWorkerActor:
    ...
    async def register_fragments(self, fragments: list[dict[str, Any]]) -> dict[str, int]:
        ...
        for entry in pending_entries:
            ...
            if plan is None:
                raise ValueError(
                    f"fragment {fragment_id} registration requires a physical plan"
                )
