...
class FteWorkerTaskControlMixin:
    ...
    def _get_fte_control_ref(
        ...
        if cancel_event is not None:
            ...
            deadline = (
                ...
                else time.monotonic() + resolved_timeout_s
            )
            future = ref.future()
            while True:
                ...
                if remaining is not None and remaining <= 0.0:
                    raise TimeoutError(
                        f"{method_name} did not complete within {resolved_timeout_s:.3f}s"
                    )
                ...
                with _FTE_REGISTRY_LOCK:
                    if query_id in _FTE_CLOSING_QUERIES:
                        if method_name == "fte_release_task_result":
                            ...
                            return None
                        raise RuntimeError(
                            f"FTE control admission is closed for query {query_id}: {method_name}"
                        )
        ...
        try:
            with self._fte_control_lock:
                if task_key in self._fte_control_tails_by_task:
                    raise RuntimeError(
                        f"FTE create task control already exists: {task_key}"
                    )
            ...
            if status.get("_fte_control_applied") is not True:
                raise RuntimeError(
                    f"FTE create task control was not applied: task={task_key}"
                )
            return status
    ...
    def fte_add_splits(
        ...
        raw_status = self._enqueue_ordered_fte_control_rpc(
            "fte_add_splits", task_id, source_node_id, splits
        )
        if not isinstance(raw_status, dict):
    ...
    def fte_no_more_splits(
        ...
        raw_status = self._enqueue_ordered_fte_control_rpc(
            "fte_no_more_splits", task_id, source_node_id
        )
        if not isinstance(raw_status, dict):
    ...
    def fte_update_task(
        ...
        raw_status = self._enqueue_ordered_fte_control_rpc(
            "fte_update_task", task_id, update
        )
        if not isinstance(raw_status, dict):
    ...
    def close_and_flush_fte_controls(self, query_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(resolved, list) or len(resolved) != len(pending):
                raise TypeError(
                    "FTE result-control barrier must return one status per task"
                )
            ...
            for entry in pending:
                ...
                if not callable(future_method):
                    ...
                    terminal_errors.append(
                        f"task={task_key}: control ref has no future()"
                    )
                    continue
                try:
                    ...
                    terminal_entries.append(entry)
                    terminal_errors.append(
                        f"task={task_key}: {type(exc).__name__}: {exc}"
                    )
                    continue
                try:
                    ...
                    try:
                        is_done = bool(
                            done_value() if callable(done_value) else done_value
                        )
                    except BaseException:
                        is_done = False
                    if is_done:
        ...
        for (task_key, _, expected_operation), raw_status in resolved_entries:
            try:
                if not isinstance(raw_status, Mapping):
                    raise TypeError(
                        "FTE result-control barrier status must be a mapping"
                    )
                status = dict(raw_status)
                ...
                if status.get("_fte_control_applied") is not True:
                    raise RuntimeError(
                        "FTE control barrier operation was not applied: "
                        f"task={task_key} operation={expected_operation}"
                    )
            except BaseException as exc:
                terminal_errors.append(
                    f"task={task_key}: {type(exc).__name__}: {exc}"
        ...
        if pending_entries:
            details = [
                "pending=" + ",".join(task_key for task_key, _, _ in pending_entries)
            ]
            if barrier_resolution_error is not None:
                details.append(
                    "wait="
                    f"{type(barrier_resolution_error).__name__}: "
                    f"{barrier_resolution_error}"
                )
            details.extend(terminal_errors)
            raise FteControlBarrierPendingError(
                f"FTE control barrier still has non-terminal operations for {query_key}: "
                + "; ".join(details)
            )
        if terminal_errors:
            details = list(terminal_errors)
            raise FteControlBarrierTerminalError(
                f"FTE control barrier reached terminal failures for {query_key}: "
                + "; ".join(details)
            )
        return statuses
    ...
    def _has_fte_control_state_for_query(self, query_id: str) -> bool:
        ...
        with self._fte_control_lock:
            return any(
                owner_query_id == query_key
                for owner_query_id in self._fte_control_query_by_task.values()
            )
    ...
    def fte_drop_query(self, query_id: str) -> dict[str, int]:
        ...
        if fence_error is not None:
            ...
            if barrier_error is not None or barrier_pending_error is not None:
                control_error = (
                    barrier_pending_error
                    if barrier_pending_error is not None
                    else barrier_error
                )
                details.append(
                    f"barrier={type(control_error).__name__}: {control_error}"
                )
            details.append(f"fence={type(fence_error).__name__}: {fence_error}")
            raise RuntimeError(
                f"FTE query teardown did not reach the remote-drop fence for {query_id}: "
                + "; ".join(details)
            ) from fence_error
        if barrier_pending_error is not None:
            raise FteControlBarrierPendingError(
                f"FTE query teardown retained pending control ownership for {query_id}: "
                f"{barrier_pending_error}"
        ...
        if barrier_error is not None or drop_error is not None or local_error is not None:
            ...
            cause = (
                local_error
                if local_error is not None
                else (barrier_error if barrier_error is not None else drop_error)
            )
            raise RuntimeError(
                f"FTE query teardown failed for {query_id}: " + "; ".join(details)
            ) from cause
        return result
