...
def _stage_key_for_partition(key: PartitionKey) -> StageKey:
    ...
    if fragment_execution is None:
        raise KeyError(
            f"FTE fragment execution {key[0]}/{key[1]} is not registered"
        )
    ...
    with _FTE_REGISTRY_LOCK:
        return {
            "probes": {
                ...
                for (resource_query_id, stage_id), partition_key in sorted(
                    _FTE_STAGE_SUBMISSION_PROBES.items()
                )
            },
            "blocks": {
                f"{resource_query_id}/{stage_id}": {
                ...
                for (resource_query_id, stage_id), blocked in sorted(
                    _FTE_STAGE_SUBMISSION_BLOCKS.items()
                )
            },
        }
