...
class QueryResourceManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._reverse_topological_stage_ids = (
            graph.reverse_topological_stage_ids()
        )
        ...
        self._downstream_fte_stage_ids_requiring_separate_slot = {
            stage_id: graph.downstream_fte_stage_ids_requiring_separate_slot(
                stage_id
            )
        ...
        for source_stage_id in self._topological_stage_ids:
            ...
            self._reachable_udf_stage_ids[source_stage_id] = tuple(
                stage_id
                for stage_id in ordered_reachable
                if self._stages[stage_id].spec.stage_kind == "udf"
            )
        ...
        self._terminal_attempts = BoundedSet[tuple[str, str]](
            capacity=_TERMINAL_IDENTITY_REPLAY_CAPACITY
        )
        ...
        self._terminal_output_blocks = BoundedSet[str](
            capacity=_TERMINAL_IDENTITY_REPLAY_CAPACITY
        )
    ...
    def try_acquire_next_queued_task(
        ...
        with self._lock:
            ...
            owned_fatal = [
                ...
                if item.fatal and item.key in candidate_keys
            ]
            if owned_fatal:
        ...
        if stage.spec.backend == "ray_actor":
            ...
            for placement in self.allocation.actor_placements:
                ...
                if self._actor_slot_lease_count_locked(slot_key) >= stage.spec.actor_prefetch_depth:
                    continue
                free_actor_indices_by_node.setdefault(placement.node_id, []).append(
                    placement.actor_index
                )
        ...
        if credit_template is not None:
            ...
            allowed_node_ids = (
                ...
                if borrowed_credit is not None
                    ...
                    set(free_actor_indices_by_node)
    ...
    def _available_continuation_credit_locked(
        ...
        *,
        ...
        allocation_by_node = {
            item.node_id: item.resources
            for item in self.allocation.node_allocations
        }
        candidates = [
            credit
            for credit in self._continuation_credits.values()
        ...
        def candidate_rank(credit: _ContinuationCredit) -> tuple[Any, ...]:
            ...
            query_exceeded = (query_usage + incremental).exceeded_dimensions(
                self.allocation.resources
            )
            node_usage = self._node_usage_locked(credit.node_id)
            node_exceeded = (node_usage + incremental).exceeded_dimensions(
    ...
    def _downstream_reservations_locked(
        ...
        allocation_node_ids = tuple(
            sorted(allocation.node_id for allocation in self.allocation.node_allocations)
        )

        inactive_fte = tuple(
            self._stages[stage_id].spec
            for stage_id in self._downstream_fte_stage_ids_requiring_separate_slot[
                parent.stage_id
            ]
            if not self._stages[stage_id].completed
        ...
        if inactive_fte:
            resources = _component_max(
                tuple(self._task_commitment(stage) for stage in inactive_fte)
            )
        ...
        def is_after_streaming_actor(stage_id: str) -> bool:
            ...
            return any(
                ...
                and stage_id in self._reachable_stage_ids[candidate.spec.stage_id]
    ...
    def _select_waiting_task_evaluation_locked(
        ...
        normal = [
            item
            for item in evaluations
            if not item.fatal and item.reason is None
        ]
        if normal:
        ...
        soft = [
            item
            for item in evaluations
            if not item.fatal and item.reason in _SOFT_TASK_BLOCK_REASONS
        ]
        if not soft:
            return None
        ...
        ordered_stage_ids = [
            stage_id
            for stage_id in self._reverse_topological_stage_ids
            if stage_id in soft_stage_ids
        ]
        starving_stage_ids = [
            stage_id
        ...
        return min(
            (
                item
                for item in soft
                if str(item.request.stage_id) == preferred_stage_id
            ),
    ...
    def _maybe_return_continuation_credit_locked(self, task_lease_id: str) -> None:
        ...
        if task_key in self._task_leases or any(
            output.task_lease_id == task_key
            and output.state not in {"downstream_input", "external_consumer"}
    ...
    def try_acquire_next_queued_output_block(
        ...
        with self._lock:
            ...
            fatal: list[
                tuple[tuple[int, int], OutputBlockRequest, str, int]
            ] = []
            normal: list[
                tuple[tuple[int, int], OutputBlockRequest, str, int]
            ] = []
            ...
            for order, (block_id, request) in enumerate(
                self._waiting_output_blocks.items()
            ):
                ...
                reason, is_fatal, delta = self._normal_output_block_reason_locked(
                    request
                )
            ...
            if not self._output_consumer_starving_locked(
                request.producer_stage_id
            ):
                return request, OutputBlockGrant(
                    False,
    ...
    def _output_consumer_starving_locked(self, producer_stage_id: str) -> bool:
        ...
        return all(
            self._active_task_count_for_stage_locked(stage_id) == 0
            for stage_id in downstream_ids
        )
    ...
    def _live_output_bytes_for_credit_locked(self, credit_id: str) -> int:
        ...
        return sum(
            lease.size_bytes
            for lease in self._output_leases.values()
            if lease.continuation_credit_id == credit_key
        )
    ...
    def _continuation_output_excess_by_stage_locked(self) -> dict[str, int]:
        ...
        for output in self._output_leases.values():
            ...
            by_stage[output.producer_stage_id] = (
                by_stage.get(output.producer_stage_id, 0) + output.size_bytes
            )

        topological_rank = {
            stage_id: index
            for index, stage_id in enumerate(self._topological_stage_ids)
        }
        ...
        for credit_id, bytes_by_stage in outputs_by_credit_and_stage.items():
            ...
            for stage_id in sorted(
                ...
                if excess > 0:
                    excess_by_stage[stage_id] = (
                        excess_by_stage.get(stage_id, 0) + excess
                    )
    ...
    def _query_usage_locked(self) -> ResourceVector:
        ...
        total = total + ResourceVector(
            object_store_bytes=self._uncovered_inactive_output_bytes_locked(active_task_ids)
        )
        return total
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            allocation_by_node = {item.node_id: item.resources for item in self.allocation.node_allocations}
            reservation_source_stage_ids = {
                lease.stage_id for lease in self._task_leases.values()
            } | {
