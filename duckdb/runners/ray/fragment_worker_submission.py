...
from duckdb.runners.fte.fte_events import SplitEventsSubmitted
from duckdb.runners.fte.fte_scheduler import FteEventDrivenTaskSource
...
from duckdb.runners.ray.fragment_worker_exchange import mark_exchange_source_partitions_seen
from duckdb.runners.ray.fragment_worker_inputs import extract_task_inputs
from duckdb.runners.ray.fragment_worker_ordering import order_fte_handles, order_fte_scheduled_handles
...
from duckdb.runners.ray.fragment_worker_selection import available_fte_workers
from duckdb.runners.ray.fragment_worker_state import fte_fragment_execution_items, get_or_create_fte_fragment_state
...
def _registered_fte_task_memory_bytes(
    ...
    if stage.backend != "ray_worker":
        raise RuntimeError(
            f"FTE stage {resource_stage_id} has invalid registered backend {stage.backend!r}"
        )
    task_memory_bytes = int(stage.per_task.heap_bytes)
    if task_memory_bytes <= 0:
        raise RuntimeError(
            f"FTE stage {resource_stage_id} has invalid per-task heap lease {task_memory_bytes}"
        )
...
class FteWorkerSubmissionMixin:
    def _ensure_fragment_progress_topology(
        ...
        def build_topology() -> dict[str, Any]:
            ...
            topology_conn = duckdb.connect(config={"threads": "1"})
    ...
    def _drain_fte_pending_tasks(self, *, query_id_filter: str | None = None) -> list[Any]:
        ...
        fragment_execution_items = [
            item
            for item in fragment_execution_items
            if not fte_registry_query_is_closing(item[0][0])
        ]
    ...
    def submit_tasks(self, tasks: list[RayWorkerTask]) -> list[Any]:
        ...
        for task in tasks:
            ...
            resource_query_id, resource_stage_id = resource_identity_from_context(
                context
            )
        ...
        if fragments_to_register:
            candidate_query_ids = sorted(
                {
                    str(entry.get("query_id") or "").strip()
                    for entry in fragments_to_register.values()
                }
            )
            if not candidate_query_ids or not candidate_query_ids[0]:
                raise RuntimeError(
                    "fragment registration requires non-empty query identity"
                )
            ...
            try:
                for registration_query_id in candidate_query_ids:
                    if not begin_fte_registry_operation(registration_query_id):
                        raise RuntimeError(
                            f"FTE query registry is closing: {registration_query_id}"
                        )
                ...
                with self._fragment_registration_lock:
                    ...
                    for fragment_id, entry in fragments_to_register.items():
                        ...
                        owner_query_id = self._fragment_query_ids.get(fragment_id)
                        if (
                            owner_query_id is not None
                            and owner_query_id != registration_query_id
                        ):
                            raise RuntimeError(
                                "fragment registration query ownership mismatch: "
                    ...
                    if fragment_payloads:
                        ...
                        registration_ref = self.actor_handle.register_fragments.remote(
                            fragment_payloads
                        )
                ...
                def finish_registrations(*, failed: bool) -> None:
                    with self._fragment_registration_lock:
                        for submitted_fragment_id in registration_fragment_ids:
                            if (
                                self._fragment_registration_refs.get(submitted_fragment_id)
                                is not registration_ref
                            ):
                                continue
        ...
        for item in pending:
            ...
            if registration_result is not None:
                item["fragment_plan"] = fragment_plan_refs_by_fragment.get(
                    item["fragment_id"]
                )
