    ...
    while True:
        with _FTE_REGISTRY_LOCK:
            watcher_items = [
                ...
                or FteTaskAttemptId.parse(attempt_key).query_id == query_key
            ]
        ...
        if alive:
            raise RuntimeError(
                "FTE status watcher shutdown timed out without losing ownership: "
                + ", ".join(sorted(alive))
            )
...
def begin_fte_registry_operation(query_id: str) -> bool:
    ...
    with _FTE_REGISTRY_CONDITION:
        if query_key in _FTE_CLOSING_QUERIES:
            return False
        _FTE_ACTIVE_OPERATIONS_BY_QUERY[query_key] = (
            _FTE_ACTIVE_OPERATIONS_BY_QUERY.get(query_key, 0) + 1
        )
        return True
...
def begin_fte_registry_teardown_operation(query_id: str) -> None:
    ...
    with _FTE_REGISTRY_CONDITION:
        if query_key not in _FTE_CLOSING_QUERIES:
            raise RuntimeError(
                f"FTE teardown operation requires a closing query: {query_key}"
            )
...
def end_fte_registry_operation(query_id: str) -> None:
    ...
    with _FTE_REGISTRY_CONDITION:
        ...
        if active <= 0:
            raise RuntimeError(
                f"FTE registry operation ownership underflow: {query_key}"
            )
        if active == 1:
...
def end_fte_registry_teardown_operation(query_id: str) -> None:
    ...
    with _FTE_REGISTRY_CONDITION:
        ...
        if active <= 0:
            raise RuntimeError(
                f"FTE registry teardown ownership underflow: {query_key}"
            )
        if active == 1:
...
def open_fte_registry_for_query(query_id: str) -> None:
    ...
    with _FTE_REGISTRY_LOCK:
        stale_owners = [
            ...
            for name, present in (
                ...
                (
                    ...
                    any(
                        partition_key[0] == query_key
                        for partition_key in _FTE_STAGE_SUBMISSION_PROBES.values()
                    ),
                ),
                (
                ...
                ("worker_reservation_generations", any(key[0] == query_key for key in _FTE_WORKER_RESERVATION_GENERATIONS)),
                ("sequences", any(key[0] == query_key for key in _FTE_SEQUENCES)),
                ("result_handles", query_key in _FTE_RESULT_HANDLES_BY_QUERY),
                (
                    "status_watchers",
                    any(
                        FteTaskAttemptId.parse(key).query_id == query_key
                        for key in _FTE_STATUS_WATCHERS
                    ),
                ),
        ...
        if stale_owners:
            raise RuntimeError(
                f"cannot reopen FTE registry with old generation state: {query_key}; "
                + ", ".join(stale_owners)
            )
...
def _fragment_plan_cache_has_query(query_id: str) -> bool:
    with _FRAGMENT_PLAN_REF_CACHE_LOCK:
        return any(
            ...
            for _, owner_query_id, _ in _FRAGMENT_PLAN_REF_CACHE
        )
...
def _drop_fte_registry_for_query(query_id: str) -> None:
    ...
    with _FTE_REGISTRY_LOCK:
        ...
        worker_handles = [
            handle for handle in _FTE_WORKER_HANDLES.values() if handle is not None
        ]
        _FTE_REGISTRY_CONDITION.notify_all()
...
def _store_fte_result_handles(query_id: str, handles: list[Any]) -> None:
    ...
    with _FTE_REGISTRY_LOCK:
        if str(query_id) in _FTE_CLOSING_QUERIES:
            raise RuntimeError(
                f"FTE query registry is closing: {query_id}"
            )
...
def _fte_execution_queries_waiting_for_resource(
    ...
    with _FTE_REGISTRY_LOCK:
        ...
        execution_query_ids = {
            ...
            if waiter_identity[0] == resource_query_key
            and execution_query_id not in closing_queries
        }
    ...
    for (execution_query_id, _fragment_id), fragment_execution in fragment_execution_items:
        if execution_query_id in closing_queries:
            continue
        owner_query_id, _stage_id = resource_identity_from_context(
            fragment_execution.context
        )
...
def drain_fte_resource_admission_change(query_id: str) -> list[Any]:
    ...
    handles: list[Any] = []
    for execution_query_id in _fte_execution_queries_waiting_for_resource(
        query_id
    ):
...
def fte_execution_query_ids_for_resource(
    ...
    execution_query_ids = {
        ...
        if resource_identity_from_context(fragment_execution.context)[0]
        == resource_query_key
    }
    return tuple(sorted(execution_query_ids))
...
def _sync_write_sink_stage_for_fragment(fragment_execution: Any) -> str | None:
    ...
    resource_query_id, stage_id = resource_identity_from_context(
        fragment_execution.context
    )
...
def _fte_fragment_resource_identity(
    ...
    with _FTE_REGISTRY_LOCK:
        fragment_execution = _FTE_FRAGMENT_EXECUTIONS.get(
            (str(query_id), str(fragment_id))
        )
    if fragment_execution is None:
        raise KeyError(
            f"FTE fragment execution {query_id}/{fragment_id} is not registered"
        )
...
def _acquire_fte_partition_task_lease(
    *,
    ...
    if grant.fatal:
        raise RuntimeError(
            f"fatal FTE task lease rejection for {query_id}/{fragment_id}/{partition_id}: "
            f"{grant.blocked_reason}"
        )
    raise FteWorkerReservationUnavailable(
        query_id=str(query_id),
        fragment_id=str(fragment_id),
...
def fte_partition_task_lease_payload(
    ...
    if lease.attempt_id != actual_attempt_id:
        raise RuntimeError(
            f"FTE task lease attempt mismatch: lease={lease.attempt_id} command={actual_attempt_id}"
        )
    from duckdb.runners.ray.query_resource_runtime import get_query_resource_manager

    stage = get_query_resource_manager(lease.query_id).graph.stage_by_id(
        lease.stage_id
    )
    return {
...
def _fte_effective_worker_memory_budget_bytes(
    ...
    if budget_bytes <= 0:
        raise RuntimeError(
            f"Ray worker {handle.worker_id or '<unknown>'} has invalid logical memory capacity "
            f"{budget_bytes}"
        )
    return budget_bytes
...
class FteWorkerPlacementManager:
    ...
    def acquire(
        ...
        *,
        ...
        eligible_node_ids = set(
            get_query_resource_manager(resource_query_id).task_eligible_node_ids(
                stage_id
            )
        )
        if not eligible_node_ids:
            raise RuntimeError(
                f"FTE stage {stage_id} has no feasible node in its query allocation"
            )
        with _FTE_REGISTRY_LOCK:
            if query_id in _FTE_CLOSING_QUERIES:
                raise RuntimeError(f"FTE query registry is closing: {query_id}")
            ...
            if owner is not None and (
                not owner._fte_healthy or str(owner.node_id) not in eligible_node_ids
            ):
            ...
            while owner is None:
                try:
                    ...
                    if (
                        owner is not None
                        and str(owner.worker_id) in excluded_worker_ids
                    ):
                        owner = None
                except Exception:
                    _release_fte_partition_task_lease_for_key(
                ...
                if owner is None:
                    ...
                    live_worker_node_ids = {
                        str(worker.node_id) for worker in available_workers
                    }
        ...
        with _FTE_REGISTRY_LOCK:
            ...
            if terminal is None and lease_record is not None:
                ...
                partition = (
                    None
                    if fragment_execution is None
                    else fragment_execution.partitions.get(partition_id)
                )
                terminal = bool(
    ...
    def release_query(query_id: str) -> int:
        ...
        with _FTE_REGISTRY_LOCK:
            owner_items = [
                (key, owner) for key, owner in list(_FTE_PARTITION_OWNERS.items()) if key[0] == query_id
            ]
...
def _fte_progress_partition_metrics(
    ...
    return {
        ...
        "running_attempts": [
            ...
            for _, attempt in sorted(partition.running_attempts.items())
            for task_stats in [
                partition.running_task_stats.get(attempt.attempt_id.attempt_id)
            ]
    ...
    with _FTE_REGISTRY_CONDITION:
        ...
        if existing is not None:
            if existing != topology:
                _FTE_REGISTRY_CONDITION.notify_all()
                raise RuntimeError(
                    f"native fragment progress topology changed after publication: {fragment_key}"
                )
            _FTE_REGISTRY_CONDITION.notify_all()
            return False
...
def wait_for_fte_query_progress_topology(
    ...
    *,
    ...
    with _FTE_REGISTRY_CONDITION:
        while not any(
            fragment_query_id == query_key
            for fragment_query_id, _ in _FTE_FRAGMENT_PROGRESS_TOPOLOGIES
        ):
            if query_key in _FTE_CLOSING_QUERIES:
            ...
            if remaining <= 0:
                raise TimeoutError(
                    f"FTE query topology initialization timed out after {timeout_s:.3f}s: {query_key}"
                )
            _FTE_REGISTRY_CONDITION.wait(remaining)
...
def _fte_fragment_progress_metrics(
    ...
    with fragment_execution._state_lock:
        ...
        partition_count = len(partitions)
        failed_count = sum(
            1 for partition in partitions.values() if partition["state"] == "FAILED"
        )
        finished_count = sum(
            1 for partition in partitions.values() if partition["state"] == "FINISHED"
        )
        return {
            "fragment_id": fragment_id,
            "fragment_execution_id": int(fragment_execution.fragment_execution_id),
...
def fte_progress_registry_snapshot(query_id: str) -> dict[str, Any]:
    ...
    with _FTE_REGISTRY_LOCK:
        fragment_execution_items = tuple(
            ...
            for (fragment_query_id, fragment_id), fragment_execution in sorted(
    ...
    if missing_topologies:
        raise RuntimeError(
            "FTE fragment execution exists without coordinator topology: "
    ...
    failed = any(
        fragment_execution["failed"]
        for fragment_execution in fragment_executions.values()
    )
...
def _fte_query_metrics(
    ...
    *,
    ...
    query_ids.update(
        query_id for query_id, _, _ in pending_worker_reservations
    )
    ...
    for query_id in sorted(query_ids):
        ...
        pending_reservation_count = sum(
            1
            for reservation_query_id, _, _ in pending_worker_reservations
            if reservation_query_id == query_id
        )
...
def fte_registry_stats() -> dict[str, Any]:
    with _FTE_REGISTRY_LOCK:
        ...
        result_handle_counts = {
            query_id: len(handles)
            for query_id, handles in _FTE_RESULT_HANDLES_BY_QUERY.items()
        }
        retry_delays = {
            query_id: delayer.remaining_delay_s()
            for query_id, delayer in _FTE_RETRY_DELAYS.items()
        }
        closing_queries = sorted(_FTE_CLOSING_QUERIES)
        active_operations_by_query = dict(_FTE_ACTIVE_OPERATIONS_BY_QUERY)
        active_teardown_operations_by_query = dict(
            _FTE_ACTIVE_TEARDOWN_OPERATIONS_BY_QUERY
        )
        exchange_selectors_by_fragment = {
            key: {
                str(source_node_id): selector.to_metrics()
                for source_node_id, selector in sorted(
                    fragment_state.exchange_source_selectors_by_source.items()
                )
            }
        ...
        registry_counts = {
            ...
            "active_registry_teardown_operation_count": sum(
                active_teardown_operations_by_query.values()
            ),
        }

    event_scheduler_stats = _FTE_SCHEDULERS.stats()
    worker_stats = {
        str(worker_id): handle.fte_pressure_stats()
    ...
    return {
        **registry_counts,
        ...
        "active_registry_teardown_operations_by_query": (
            active_teardown_operations_by_query
        ),
