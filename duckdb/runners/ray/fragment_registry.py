...
class _FteWorkerPressure:
    ...
    def drop_query(self, query_id: str) -> None:
        ...
        self.running_attempts = {
            attempt for attempt in self.running_attempts if not owned_attempt(attempt)
        }
        self.reserved_partitions = {
            reservation
            for reservation in self.reserved_partitions
            if reservation[0] != query_id
        }
        self.split_counts_by_attempt = {
            attempt: count
            for attempt, count in self.split_counts_by_attempt.items()
            if not owned_attempt(attempt)
        }
        self.split_bytes_by_attempt = {
            attempt: count
            for attempt, count in self.split_bytes_by_attempt.items()
            if not owned_attempt(attempt)
        }
        self.memory_bytes_by_attempt = {
            attempt: count
            for attempt, count in self.memory_bytes_by_attempt.items()
            if not owned_attempt(attempt)
        }
        self.memory_bytes_by_reservation = {
            reservation: count
...
_FTE_ACTIVE_TEARDOWN_OPERATIONS_BY_QUERY = (
    _FTE_REGISTRY_STATE.active_teardown_operations_by_query
)
