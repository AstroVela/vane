...
class FteWorkerPressureAccountingMixin:
    def finish_fte_task_with_outputs(
        ...
        if (
            execution_query_id != attempt.task_id.query_id
            or lease_attempt_id != str(attempt)
        ):
    ...
    def set_fte_task_execution_class(
        ...
        with _FTE_REGISTRY_LOCK:
            ...
            if not old_class.can_transition_to(new_class):
                raise ValueError(
                    f"cannot change FTE task execution class from {old_class.value} to {new_class.value}"
                )
