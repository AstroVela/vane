    ...
    def _has_fragment_registration_state_for_query(self, query_id: str) -> bool:
        ...
        with self._fragment_registration_lock:
            return any(
                ...
                for owner_query_id in self._fragment_query_ids.values()
            )
    ...
    def drop_query_fragments(self, query_id: str) -> int:
        ...
        if remote_error is not None or local_error is not None:
            ...
            if remote_error is not None:
                details.append(
                    f"remote={type(remote_error).__name__}: {remote_error}"
                )
            ...
            raise RuntimeError(
                f"fragment teardown failed for {query_id}: " + "; ".join(details)
            ) from cause
        return removed
        ...
        try:
            ...
            with self._fragment_registration_lock:
                ...
                if pending_ref is not None:
                    ...
                    fragment_id not in self._registered_fragment_ids
                    and fragment_plan is not None
                ):
    ...
    def fte_attempt_is_selected(self, attempt_id: Any) -> bool:
        ...
        with _FTE_REGISTRY_LOCK:
            candidates = [
                ...
                and int(fragment_execution.fragment_execution_id)
                == int(attempt.task_id.fragment_execution_id)
            ]
            if len(candidates) != 1:
                raise RuntimeError(
            ...
            return bool(
                partition.finished
                and partition.selected_attempt == attempt.attempt_id
            )
