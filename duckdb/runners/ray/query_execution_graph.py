...
class ResourceVector:
    ...
    def exceeded_dimensions(self, capacity: ResourceVector) -> tuple[str, ...]:
        return tuple(
            name for name in self._FIELDS if getattr(self, name) > getattr(capacity, name)
        )
...
class QueryAllocation:
    ...
    def __post_init__(self) -> None:
        ...
        if not _resource_vectors_equivalent(aggregate, self.resources):
            raise ValueError(
                "query allocation resources must equal the sum of node_allocations"
            )
        placement_keys = [
            (placement.stage_id, placement.actor_index)
            for placement in actor_placements
        ]
        ...
        unknown_nodes = sorted(
            {placement.node_id for placement in actor_placements} - set(node_ids)
        )
        if unknown_nodes:
            raise ValueError(
                "actor placement references unallocated node_id: " + ", ".join(unknown_nodes)
            )
    ...
    def actor_node_ids_for_stage(self, stage_id: str) -> tuple[str, ...]:
        ...
        placements = sorted(
            (
                placement
                for placement in self.actor_placements
                if placement.stage_id == stage_key
            ),
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> QueryAllocation:
        ...
        return cls(
            ...
            node_allocations=tuple(
                NodeResourceAllocation.from_dict(item)
                for item in values["node_allocations"]
            ),
            actor_placements=tuple(
                ActorPlacement.from_dict(item)
                for item in values["actor_placements"]
            ),
...
class QueryExecutionGraph:
    ...
    def _validate_stage(self, stage: StageResourceSpec) -> None:
        ...
        if stage.backend == "ray_actor":
            ...
            if stage.max_concurrency is not None:
                raise ValueError(
                    f"ray_actor stage {stage.stage_id} concurrency is owned by concrete actor slots"
                )
            ...
            raise ValueError(
                f"resident_per_actor is only valid for ray_actor stages: {stage.stage_id}"
            )
        if stage.backend == "ray_task" and stage.max_concurrency is not None:
            raise ValueError(
                f"ray_task stage {stage.stage_id} concurrency is owned by resource credit"
            )
    ...
    def downstream_fte_stage_ids_requiring_separate_slot(
        ...
        downstream: dict[str, set[str]] = {
            stage_id: set() for stage_id in by_id
        }
        for stage in self.stages:
            for input_stage_id in stage.input_stage_ids:
                downstream[input_stage_id].add(stage.stage_id)

        crossed_non_fte: dict[str, bool] = {
            source.stage_id: source.backend != "ray_worker"
        }
        ...
        for stage_id in ordered:
            ...
            for child_id in downstream[stage_id]:
                ...
                crossed_non_fte[child_id] = (
                    crossed_non_fte.get(child_id, False) or child_crossed
                )
        ...
        *,
        ...
        for stage in self.stages:
            task_commitment = ResourceVector(
                ...
                object_store_bytes=(
                    stage.per_task.object_store_bytes + stage.output_window_bytes
                ),
            )
            actor_resident = (
                stage.resident_per_actor
            ...
            if (
                ...
                and output_window > capacity.object_store_bytes
            ):
                raise ValueError(
            ...
            if require_full_minimum and not task_commitment.is_zero() and not any(
                placement_commitment.fits_within(node_allocation.resources)
                for node_allocation in allocation.node_allocations
            ):
                raise ValueError(
                    f"stage {stage.stage_id} maximum task does not fit any allocated Ray node"
                )
            if stage.backend == "ray_actor":
                ...
                if require_full_minimum and exceeded_actor:
                    raise ValueError(
                        f"stage {stage.stage_id} actor minimum exceeds query allocation for "
                        f"{', '.join(exceeded_actor)}"
                    )
                placements = [
                    placement
                    for placement in allocation.actor_placements
                    if placement.stage_id == stage.stage_id
                ]
                if require_full_minimum and len(placements) != stage.actor_min_size:
                    raise ValueError(
                        f"stage {stage.stage_id} requires exactly {stage.actor_min_size} actor placements"
                    )
                ...
                if placements and placement_indices != expected_indices:
                    raise ValueError(
                        f"stage {stage.stage_id} actor placement indices must be contiguous from zero"
                    )
                for placement in placements:
                    if not placement_commitment.fits_within(
                        allocation.resources_for_node(placement.node_id)
                    ):
                        raise ValueError(
        ...
        if unknown_actor_stages:
            raise ValueError(
                "actor placement references non-actor stage: " + ", ".join(unknown_actor_stages)
            )
        if require_full_minimum:
            ...
            for placement in allocation.actor_placements:
                ...
                invocation = ResourceVector(
                    object_store_bytes=(
                        stage.per_task.object_store_bytes
                        + stage.output_window_bytes
                    )
                )
            ...
            for node_id, commitment in actor_commitment_by_node.items():
                if not commitment.fits_within(allocation.resources_for_node(node_id)):
                    raise ValueError(
                        "cumulative actor placements do not fit allocated Ray node "
                        f"{node_id}"
                    )
