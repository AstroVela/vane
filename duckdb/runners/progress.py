    ...
    *,
    ...
    for raw_pipeline in raw_pipelines:
        if type(raw_pipeline) is not dict or set(raw_pipeline) != required_fields:
            raise ValueError(
                "pipeline topology entries must contain exactly "
                "pipeline_id, operators, operator_details, and stage_ids"
            )
    ...
    for stats in stats_items:
        for pipeline in stats.get("pipelines") or []:
            ...
            if item is None:
                ...
                raise RuntimeError(
                    f"pipeline {pipeline_id} changed structure while aggregating progress"
                )
...
def _pipeline_stats_with_progress_topology(
    ...
    topology_by_id = {
        pipeline["pipeline_id"]: pipeline for pipeline in topology["pipelines"]
    }
    zero_pipelines = [
        {
    ...
    for stats in live_stats_items:
        ...
        for pipeline in stats.get("pipelines") or []:
            ...
            if planned is None:
                raise RuntimeError(
                    f"live progress reported unknown pipeline_id {pipeline_id}"
                )
            operators = pipeline.get("operators")
            if operators != planned["operators"]:
                raise RuntimeError(
                    f"live progress pipeline {pipeline_id} operators do not match topology"
                )
...
def format_progress_snapshot(snapshot: dict[str, Any], *, width: int = 100) -> list[str]:
    ...
    for fragment in fragments:
        lines.append(
            f"Fragment {fragment['display_id']} "
            f"[PENDING {_format_count(fragment['pending_partitions'])}]"
        )
        lines.extend(
...
class ProgressRenderer:
    ...
    def _force_finished_snapshot(snapshot: dict[str, Any]) -> dict[str, Any] | None:
        ...
        already_finished = (
            str(snapshot.get("state") or "").upper() == "FINISHED"
            and no_active_work
            and completed_work
        )
        if already_finished:
