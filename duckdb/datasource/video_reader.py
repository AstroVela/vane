...
def _decode_video_batches(
    ...
    *,
    ...
    resize_executor = (
        ThreadPoolExecutor(max_workers=_VIDEO_RESIZE_THREADS) if _VIDEO_RESIZE_THREADS > 1 else None
    )
...
def _video_source_udf_backend() -> str:
    ...
    if backend not in {"ray_task", "subprocess_task"}:
        raise ValueError(
            "VANE_VIDEO_SOURCE_UDF_BACKEND must be one of: ray_task, subprocess_task; "
            f"got {backend!r}"
        )
    return backend
